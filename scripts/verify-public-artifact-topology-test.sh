...
awk '!/^\| fireweed-core \|/' "$source_document" >"$omitted_document"
...
grep -q 'workspace packages omitted from topology: fireweed-core' "${test_dir}/omitted.err"

duplicate_document="${test_dir}/duplicate.md"
awk '{ print; if (/^\| fireweed-core \|/) print }' "$source_document" >"$duplicate_document"
...
grep -q 'duplicate workspace package classification: fireweed-core' "${test_dir}/duplicate.err"
