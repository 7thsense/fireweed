...
chart_name="$(sed -n 's/^name:[[:space:]]*//p' charts/pqueue/Chart.yaml | head -n 1)"
chart_version="$(sed -n 's/^version:[[:space:]]*//p' charts/pqueue/Chart.yaml | head -n 1)"
chart_app_version="$(sed -n 's/^appVersion:[[:space:]]*"\{0,1\}\([^"[:space:]]*\)"\{0,1\}[[:space:]]*$/\1/p' charts/pqueue/Chart.yaml | head -n 1)"
...
require_value "charts/pqueue/Chart.yaml name" "$chart_name"
require_value "charts/pqueue/Chart.yaml version" "$chart_version"
require_value "charts/pqueue/Chart.yaml appVersion" "$chart_app_version"
...
printf 'charts/pqueue/Chart.yaml: name=%s version=%s appVersion=%s; treatment=independent source defaults; packaged release overrides version and appVersion to %s\n' \
    "$chart_name" "$chart_version" "$chart_app_version" "$target_version"

printf 'scripts/release/package-helm-chart.sh: input=--version; outputs=version,app_version,release_tag,release_asset_coordinate; evidence=pqueue-helm-chart.txt; treatment=release-synchronized; target=%s\n' \
    "$target_release"
