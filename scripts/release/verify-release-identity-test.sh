...
bash "${SCRIPT_DIR}/verify-release-identity.sh" --version 0.30.0

# Inventory classifies independent coordinates and reports tag reservation.
inventory="$(bash "${SCRIPT_DIR}/list-public-version-sources.sh" v0.30.0)"
...
printf '%s\n' "$inventory" | grep -Fq 'git tag v0.30.0: state=absent' ||
    fail "tag reservation absent state missing"
printf '%s\n' "$inventory" | grep -Fq 'Cargo.toml: workspace.package.version=0.30.0; treatment=release-synchronized' ||
    fail "workspace synchronized classification missing"

# Negative: wrong expected version fails.
if bash "${SCRIPT_DIR}/verify-release-identity.sh" --version 0.29.3 >/dev/null 2>"${CASE_ROOT}/wrong-version.err"; then
    fail "expected version mismatch to fail"
fi
grep -Fq 'workspace.package.version=0.30.0 != 0.29.3' "${CASE_ROOT}/wrong-version.err" ||
