...
import json, sys
d=json.loads(next(x for x in open(sys.argv[1], encoding="utf-8") if x.strip()))
f=json.load(open(sys.argv[2], encoding="utf-8"))
if d.get("measurements",{}).get("revision") != sys.argv[3] or f.get("source_revision") != sys.argv[3]: raise SystemExit("E2 exact revision mismatch")
PY
