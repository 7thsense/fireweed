...
def validate_document(document: object, *, check_repository: bool) -> None:
    ...
    require(deferred["retired"] is True, "SQLite deferred flush must remain retired")
    require(
        deferred["rejected_when_present"] is True,
        "retired deferred flush must be rejected when present",
    )
    ...
    require(turso["version"] == "0.7.2", "Turso version boundary drift")
    require(
    ...
    require(
        rejected == {"inmemory", "hybrid", "hybrid-strict", "hybrid-async", "sqlite"},
        "projection legacy rejection drift",
    )
    require(
        bijections["log"]["rejected_legacy_values"] == ["objectlog", "sqlite"],
    ...
    require(formula["postgres_log_strict_enabled"] == 3 * 1 * 3, "relational profile count")
    require(formula["filesystem_or_s3_log_both_barriers_enabled"] == 6 * 2 * 3, "object profile count")
    require(formula["total"] == 45, "TD-008 positive profile count")
