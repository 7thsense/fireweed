...
TD-004's `hybrid-async` bounded-debt SQLite projection, and provisions 1,001 queues
...
`hybrid-async` selects the supported manifest group-commit path. Success remains
anchored by the durable object-log manifest; SQLite apply debt is bounded by
TD-004's governed lag, byte, queue-depth, age, and poison thresholds instead of
putting an unrelated host's synchronous SQLite latency on every response.
