...
**Suite:** `crates/fireweed-server/tests/performance_single_deployment_baseline_tests.rs`
...
```
FIREWEED_PERF_ENV=1 FIREWEED_E1_RESIDENT=10000000 FIREWEED_E1_FULL=1 \
  FIREWEED_E0E1_PROGRESS_BOUND_MS=<positive-workload-declaration> \
  FIREWEED_PG_TEST_URL=postgres://postgres:fireweed@<instance>:5432/postgres \
  cargo test -p fireweed-server --features postgres --test performance_single_deployment_baseline_tests --release -- --nocapture
```
...
- The claim selection uses the partial index `fireweed_items_claim_idx (tenant_id, queue_id, priority_sort,
...
provisioned `postgres_native` deployment must tune autovacuum aggressively on `fireweed_items`.**
...
Emitted by the suite to `target/fireweed-ledger/` (the CI gate points `$FIREWEED_LEDGER_DIR` at a collection
