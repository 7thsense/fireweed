...
The release evidence passed. A three-replica pqueue deployment retained all four visible items across active-owner deletion, advanced ownership from epoch 2 to epoch 3, rejected the stale writer before mutation, and admitted no double lease or corrupt write.

## Reproduction

```bash
PQUEUE_E2_SEED=2002 bash scripts/perf/tp002-e2-failover-kind.sh
```
...
- Runtime image: `pqueue:e2-failover`, image ID `sha256:a6e3aa6699500c8334019f5308b95b10761191a7a5b97c97d3e0ad0163d891b3`
...
- Topology: three pqueue pods, shared MinIO object log, PostgreSQL ownership, and one SQLite projection per pod
