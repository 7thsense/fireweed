...
artifacts. Replace `OWNER`, `REPO`, and `v0.2.2` with the release repository and
...
For example, release tag `v0.2.2` uses version `0.2.2`, so the chart package is
`pqueue-0.2.2.tgz` and binary archives are named like
`pqueue-0.2.2-x86_64-linux.tar.gz`. The `v0.2.2` workflow publishes the Helm
...
```text
https://github.com/OWNER/REPO/releases/tag/v0.2.2
```
