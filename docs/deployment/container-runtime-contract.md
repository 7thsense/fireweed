...
| `PQUEUE_PROJECTION_BACKEND` | no | `inmemory` | Projection backend axis: `inmemory`, `sqlite`, `hybrid`, or `postgres`. |
...
| `PQUEUE_SQLITE_PROJECTION_PATH` | when projection is `sqlite` or `hybrid` | `/var/lib/pqueue/pqueue-projection.db` | Local SQLite materialized projection path for `objectlog/sqlite` and the SQLite-first durable image for `objectlog/hybrid`. |
