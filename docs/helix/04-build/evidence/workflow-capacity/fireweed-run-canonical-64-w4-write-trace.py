...
output=repo/'target/workflow-capacity/canonical-64-w4-write-trace-20260916.json'
...
log=output.open('w')
runner=subprocess.Popen(['python3','scripts/perf/workflow-capacity.py','--profile','campaign','--campaign-metadata','--campaign-timestamp-priority','--items','1000000','--batch','1000','--purge-batch','8000','--shards','64','--workers','4','--load-workers','2','--recycle','--cycles','8','--deadline-seconds','1800','--stretch'],cwd=repo,stdout=log,stderr=subprocess.STDOUT)
active=None
phases=[]
def finish_active():
 global active
...
try:
 while runner.poll() is None:
  ...
  if pid is not None and active is None:
   base=Path(f'/tmp/fireweed-canonical-64-w4-write-trace-phase-{len(phases)+1}')
 ...
 finish_active()
 log.close()
 Path('/tmp/fireweed-canonical-64-w4-write-trace-phases.json').write_text(json.dumps({'runner_exit':runner.poll(),'output':str(output),'phases':phases,'host_counters_are_not_process_attribution':True,'initial_and_final_observation_gaps_possible':True},indent=2)+'\n')
