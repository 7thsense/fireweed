...
output=repo/'target/workflow-capacity/joint-log-projection-trace-20260916.json'
...
try:
 while runner.poll() is None:
  ...
  if pid is not None and active is None:
   base=Path(f'/tmp/fireweed-joint-log-projection-trace-phase-{len(phases)+1}')
 ...
 finish_active()
 log.close()
 Path('/tmp/fireweed-joint-log-projection-trace-phases.json').write_text(json.dumps({'runner_exit':runner.poll(),'output':str(output),'phases':phases,'host_counters_are_not_process_attribution':True,'initial_and_final_observation_gaps_possible':True},indent=2)+'\n')
