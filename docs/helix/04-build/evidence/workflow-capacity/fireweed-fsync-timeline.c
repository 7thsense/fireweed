...
static int timed_sync(int fd, int data_only) {
    ...
    if (measured) {
        ...
        int count = snprintf(output, sizeof(output), "log_sync class=%s kind=%s call=%s us=%" PRId64 " start_ns=%" PRId64 " end_ns=%" PRId64 " result=%d\n",
            class, directory ? "directory" : "file", data_only ? "fdatasync" : "fsync", ns/1000, (int64_t)before.tv_sec*1000000000+before.tv_nsec, (int64_t)after.tv_sec*1000000000+after.tv_nsec, result);
