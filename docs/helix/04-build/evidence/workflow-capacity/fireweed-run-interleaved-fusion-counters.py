...
assert head.startswith('7e999e9d')
assert not subprocess.check_output(['git','status','--porcelain'],cwd=repo).strip()
base=repo/'target/workflow-capacity/interleaved-fusion-counters-20260916'
...
recorder=Path('/tmp/fireweed-interleaved-fusion-counter-recorder.py');recorder.write_text(source)
binaries=[repo/'target/release/fireweed-workload',Path('/tmp/fireweed-workload-before-claim-identity')]
def digest(p):
 with p.open('rb') as f:return hashlib.file_digest(f,'sha256').hexdigest()
hashes=[digest(p) for p in binaries]
assert hashes[1]=='5d0ac2955435d238bb631b9e432e8834c6fe5d9f53964825f70d98097df9be8f'
...
records=[]
for name,index,origin in [('candidate-1',0,head),('control',1,'fa49f380242c8319d3f00143cda848bfea81a5b9'),('candidate-2',0,head)]:
 ...
 provenance={'diagnostic_only':True,'build_source_head':origin,'executable_sha256':hashes[index],'recorder_source_head':head,'recorder_sha256':digest(recorder),'purpose':'one-cycle interleaved-fusion hardware-counter and projection-write comparison; perf counts workload only; not sustained qualification'}
