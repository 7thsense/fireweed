...
size=8*1024**3
path.touch()
# Normal project filesystem attributes, unchanged.
attributes=subprocess.check_output(['lsattr','-d',str(path)]).decode().strip()
assert 'C' not in attributes.split()[0]
...
with path.open('rb') as f:
 ...
 f.seek(size-chunk)
 assert hashlib.sha256(f.read(chunk)).hexdigest()==expected
report={'diagnostic':True,'purpose':'normal project filesystem buffered sequential writes with final fdatasync; not a workflow benchmark',
 ...
 'direct_io_requested':False,'fdatasync_included':True,'wall_s':wall,'fdatasync_s':flush_s,
