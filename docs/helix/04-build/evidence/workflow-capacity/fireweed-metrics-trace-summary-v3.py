...
for s in strings:
 for m in re.finditer(r'metrics_read path=(\w+) phases_us=\[([\d, ]+)\]',s):
  ...
  rows.append((m[1],values+[0]*(11-len(values))))
names=['high_water','first_admission','snapshot_sql','claim_tail','coverage','second_admission','final_sql','membership_tail','membership_sql','lifecycle_tail','lifecycle_sql']
def summary(rows):
 ...
 'dominant_phase_counts':dict(collections.Counter(names[max(range(11),key=lambda i:r[1][i])] for r in rows))}
