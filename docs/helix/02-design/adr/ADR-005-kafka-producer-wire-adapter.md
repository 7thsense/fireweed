...
Phase 6).** The Kafka producer wire adapter and its crate `fireweed-kafka` have been **DELETED**. The
...
`fireweed` crate) — and Kafka compatibility is not among them (ADR-007 §"Interface sprawl"). This ADR is
...
embedded in fireweed-server (ADR-014, decided 2026-07-06). That surface has none of the data-plane
...
fireweed's PRD (Non-Goals) states that fireweed will not implement AMQP, Kafka, or
SQS compatibility as the **core data model**. This correctly forecloses a design
where fireweed's items, priorities, and progress semantics would be shaped around
Kafka's partition/offset model.

However, IP-001 Slices 7-8 identify a concrete use case: fireweed should accept
...
that engine requires only mapping Produce records onto fireweed's enqueue API —
the core data model remains fireweed-native.

## Decision

fireweed will implement a Kafka **producer** wire adapter as a P2 feature:

- **Wire protocol**: ApiVersions, Metadata, Produce requests handled by
  heimq-wire (no custom framing or handler registry).
- **Mapping**: each ProduceRequest record batch → one or more fireweed `enqueue`
...
- **Consumer-side APIs are permanently out of scope**: Fetch, ListOffsets,
  ...
  implemented. fireweed has its own native claim/fetch API; Kafka consumer
...
- **Scope boundary**: the adapter translates the wire representation only.
  fireweed's priority, scheduling, group selection, and idempotency rules apply
...
conflict with fireweed's mutable priority and progress-bound model. A fireweed item
...
require either lying to consumers or breaking fireweed invariants.
...
Viable as a deployment option but outside fireweed's scope. Does not enable
fireweed-native priority routing, group selection, or idempotency keys from
producer metadata.

## Consequences

- The PRD Non-Goals section is updated to clarify: core data model remains
  fireweed-native; the Kafka producer wire adapter is P2 (this ADR).
- A new bead will track the producer front-end implementation, gated behind
  the fireweed enqueue API being in place.
- Dependent: heimq-fef9406f (IP-001 Slices 7-8) adopts this adapter as the
  fireweed Kafka produce path conformance target.
