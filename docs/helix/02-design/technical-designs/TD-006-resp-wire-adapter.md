...
- Pending replacement is required on the public cell `s3--turso` (ADR-024). That cell must perform deterministic
...
| Pending-item replacement | pass on `s3--turso` (`XADD` with `client_item_key`), with deterministic commit-time revalidation and `AsyncProjection` | pass |
