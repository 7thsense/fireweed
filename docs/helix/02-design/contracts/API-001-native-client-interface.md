...
ddx:
  ...
  review:
    self_hash: ae6c682dbf6e269b6792351f1677477f2324fb24cb4cc4f85392f6369fd43b0b
    deps:
...
| operation | addressed entries or ordered selectors | yes | Addressed entries name an item and MAY supply caller CAS. Selectors target live or retained records using composable all/any/not predicates over lifecycle, attempt count, active lease at `evaluated_at`, not-before time, client key, group key, caller field/metadata/entity equality, and gate-key presence. An implementation MUST evaluate a valid selector even when no declared index exists. |
| lease guard | reject active, require active, match token, or invalidate | yes | Active leases MUST never be bypassed implicitly. Active means `lease_expires_at > evaluated_at`; equality is expired. Invalidation atomically removes the active or stored-expired lease and its token/index state in the same commit. |
