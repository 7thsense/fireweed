...
| AC-TURSO-2 Full differential corpus | Apply every supported queue command/history to Turso, including rollback injection, then close/reopen (no rusqlite twin) | Public projection compatibility | 0 mismatches in cursor, counters, replay outcomes, reads, eligibility, leases, indexes, summaries, or errors across apply vs reopen |
