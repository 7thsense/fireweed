...
| PRD P0 queue-density item | PRD / TD-001 / TD-002 / TD-003 / TD-004 | E2 queue density: the release command `scripts/perf/tp002-e2-density-kind.sh` proves exactly 1000 cold queues plus one hot queue on one live objectlog/SQLite node using the canonical 300,000-item/8-hot-connection/8-cold-worker/4-server-worker/seed-42 run. Every cold queue retains an eligible item (`XLEN > 0`) and completes a claim/finalize operation that starts during the hot phase within 60 seconds; the hot queue preserves the E0 ingest and claim/finalize floors with ≥100% noisy-neighbor retention. Between explicit hot-phase markers, a separate sampler bounds Tokio's actual worker pool (4), live async tasks (64), and established connections (32). The row is bound to a clean full Git revision and exact loaded image digest, with clean-HEAD checks repeated at emission and validation. This density proof excludes aggregate 1000× throughput and failover, which is tracked by `pqueue-0a1d4386`. `queue_density_single_node_tests` remains smoke-only supporting evidence. |
