...
| PRD P0 queue-density item | PRD / TD-001 / TD-002 / TD-003 / TD-004 | E2 queue density: the release command `scripts/perf/tp002-e2-density-kind.sh` proves ≥1000 concurrently active queues on one live objectlog/SQLite node. Every cold queue retains an eligible item (`XLEN > 0`) and completes claim/finalize during the hot phase within 60 seconds; the hot queue preserves the E0 ingest and claim/finalize floors with ≥100% noisy-neighbor retention. A separate sampler bounds actual server threads (64), established connections (32), and file descriptors (256). The row is bound to a clean full Git revision and the exact loaded image digest. This density proof excludes aggregate 1000× throughput and failover, which is tracked by `pqueue-0a1d4386`. `queue_density_single_node_tests` remains smoke-only supporting evidence. |
