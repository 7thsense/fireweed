...
| How stateless containers rebuild local hot state after restart, failure, or shard movement. | Covered for both backends: Postgres-native via Postgres projection; object-log via SQLite snapshot + bounded log replay; shard movement recovery via TD-003. | TD-002, TD-003, TD-004 |
| How the storage model supports batch push, batch update, batch claim, group-aware claim, lease expiry, progress-bound enforcement, per-group active-scope discovery (cross-shard ranking), and retention. | Covered for the native contract and both v1 backends. | API-001, ADR-004, TD-001, TD-002, TD-004 |
| At what domain claim result order is guaranteed (effective claim domain; a single `group_key` on a `group_co_residency=true` queue gives exact per-group order; a non-co-resident `group_key` is a restriction filter only and may return a partial group), that `shard_id` is never a client-visible ordering/progress scope, and that group co-residency is a placement capability carried in the control-plane/storage schema. | Covered. | ADR-004, API-001, TD-001, TD-002 |
| How all-or-nothing complete-cohort claims are gated on completeness + per-member eligibility, leased atomically under one shared lease, placed via `group_key` co-residency, expired against a cohort-liveness bound enforced `<= progress_bound_ms` (without a second progress scope), idempotent under duplicate push, and prevented from leaking members to other claim units, across both v1 backends. | Covered for the native contract and both backends (Postgres-native TD-002 and object-log + SQLite TD-004) via opt-in `cohort_policy`. | API-001, ADR-004, TD-001, TD-002, TD-004 |
| How recurring / never-terminal item lifecycle works: `rearm` outcome (caller-supplied eligibility materialized as `max(commit_time, not_before)` fed into the shared Eligibility Precedence timing stage, attempt-exempt, per-cycle retry reset), explicit termination (terminal finalize / `recurrence.until` drain / targeted in-band `PurgeItems` with tombstone + replay semantics, distinct from the P1 operator purge contract), recurring metrics served from the metrics projection (idle vs active, excluded from retry backlog/failure and from oldest eligible age while idle), queue-global (not per-group) progress parity, group co-residency placement (D2), cohort / recurrence mutual exclusivity (ADR-004), gate composition, active-discovery parity via `pqueue_group_summary`, and multi-shard purge split/best-effort/replay semantics. | Covered for the native contract and both backends. | API-001, ADR-004, TD-001, TD-002, TD-004 |
| Whether the headline horizontal-scale claim is substantiated by a buildable, benchmarked mechanism rather than by `postgres_native` alone. | Covered by committed v1 multi-shard claim + sharding & shard ownership + object-log backend, each with a benchmark evidence record. Tracked until E2/E3 prove the per-queue floor E0 (>=10M items/hr per queue, preserved for every queue at any scale). | product-vision, PRD, ADR-001, TD-001, TD-003, TD-004, TP-002 |
| Whether a single node supports the required queue density (>=1000 concurrently active queues) without per-queue resource blow-up or cross-queue degradation. | Covered by the bounded shared per-node resource rule: per-queue / per-`(queue,shard)` background work (lease-expiry sweeps, cross-shard progress aggregation, summary recompute, recurring rearm, idempotency/retention GC) multiplexed onto bounded shared pools, batched per-node lease renewal, and LRU-bounded per-shard projection handles. Tracked until `queue_density_single_node_tests` (TP-002 E2) passes. | PRD, ADR-002, ADR-003, TD-001, TD-002, TD-003, TD-004, TP-002 |
