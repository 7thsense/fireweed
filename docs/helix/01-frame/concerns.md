...
ddx:
  ...
  depends_on:
    ...
    - prd
  review:
    self_hash: 52b6bbb92cff001a75227115afb20f4d0a73781ec98f49ab446a6866c17284dc
    deps:
...
Project Concerns declare active cross-cutting context for downstream fireweed
...
| `api-style` | library | `area:api` | fireweed will need a native API and likely compatibility adapters, so contract shape, versioning, errors, and input validation must stay consistent. | Publish typed/versioned contracts; validate inputs at the boundary; use standard error shapes; keep wire contracts separate from the internal queue model; record non-default API style choices in ADRs. |
| `security-owasp` | library | `area:api`, `area:data`, `area:infra` | fireweed accepts untrusted payloads, metadata, API requests, and dependency inputs; a future service form must not leak secrets, internal errors, or cross-queue data. | Validate all external inputs; parameterize queries; audit dependencies; keep secrets out of source; enforce TLS for network surfaces; avoid implementation-detail leakage. |
| `auth` | library | `area:api`, `area:data`, `area:infra` | fireweed must be designable as a service where queue access is scoped by authenticated principals, not only by a single outer gate. | Model principals/accounts; derive scope from the authenticated principal; keep auth provider swappable; ensure protected operations are exercised end to end when a service surface exists. |
...
| `deployment-topology` | library | `area:api`, `area:data`, `area:infra`, `area:testing` | fireweed is expected to run like Niflheim where possible: stateless service containers behind a load balancer, tenant/queue shards assigned to workers, and durable state externalized or persisted through simple storage primitives. | Keep compute nodes stateless or rebuildable; make shard ownership and reassignment explicit; avoid mandatory external coordinators or embedded consensus unless an ADR proves they are necessary; test failover, replay, and noisy-neighbor isolation. |
| `rust-cargo` | library | `area:core`, `area:api`, `area:data`, `area:infra`, `area:testing` | fireweed is a high-performance, bounded-memory, reliability-sensitive system; Rust gives memory safety, predictable resource control, and strong concurrency tooling. | Use a pinned stable Rust toolchain; organize as a Cargo workspace; deny unsafe by default; use explicit error handling; run fmt, clippy, cargo-deny, cargo-machete, tests, benchmarks, and loom for critical concurrency. |
...
|---------|----------|----------|-----------|
| `auth` | Real signup/login/session surface | fireweed may first ship as a library or machine-facing service, so human signup/login is not assumed. Service mode uses provider-neutral principal resolution and operation-scoped authorization. | ADR-002 |
...
| `deployment-topology` | Modular service or app deployment chosen during implementation | The preferred topology is a Rust service of stateless containers behind a load balancer, with tenant/queue shards allocated through a Postgres control plane and fenced by assignment epoch. fireweed does not require ZooKeeper, etcd, or an internally maintained consensus protocol for the first backend. | ADR-001, TD-001, TD-002 |
...
- `area:api` - native fireweed APIs, client SDK surfaces, and compatibility
...
| `enterprise-integration-patterns` at-least-once delivery vs. consumer side effects | fireweed guarantees at-least-once with a single active lease; downstream consumers must remain idempotent. |
...
| `api-style` compatibility adapters vs. fireweed native model | Compatibility APIs may mirror SQS or other systems, but they cannot force the core model to drop priority updates, progress bounds, or group-aware claims. |
| `security-owasp` input validation vs. opaque payloads | Payloads may be opaque to fireweed, but queue-owned fields, metadata predicates, priority values, and API envelopes must be validated at the boundary. |
...
| Whether a Niflheim-like topology can satisfy fireweed without a mandatory coordinator or embedded consensus. | Covered through Postgres control-plane assignments and backend epoch fencing. | ADR-001, TD-001, TD-002 |
...
| Whether SQLite with WAL persisted to object storage can meet durability, replay, compaction, and write-amplification requirements. | Historical question. The public cell is S3 object-log × Turso (ADR-024). Class A durability is the object log. SQLite is not the projection and not a differential gate. | ADR-024, TD-010 |
| How native-async and blocking stores share one runtime-neutral contract without blocking a reactor, losing transaction affinity, or weakening cancellation outcomes. | Covered by typed async storage operations, explicit whole-transaction blocking adapters, and cancellation conformance. | ADR-015, TD-001, TP-003 |
| Whether Turso can serve as a Rust-native derived projection without becoming an authority or multiplying CI scale. | Turso is the public projection on the s3 × turso cell (ADR-024, ADR-016). It is not the command log. Enablement is not blocked on a SQLite differential. Recovery is `projection_control` replay of the object log. | ADR-024, ADR-016, TD-010, TP-003 |
| How stateless containers rebuild local hot state after restart, failure, or owner reassignment. | On the public cell, Turso is disposable. Rebuild is `projection_control` replay of the S3 object log (ADR-024). SQLite snapshot replay is not the current path. | ADR-024, TD-003, TD-010 |
...
| How all-or-nothing complete-cohort claims are gated on completeness + per-member eligibility, leased atomically under one shared lease, owner-local by construction (every `group_key`'s members on the queue's owner), expired against a cohort-liveness bound enforced `<= progress_bound_ms` (without a second progress scope), idempotent under duplicate push, and prevented from leaking members to other claim units, across both v1 backends. | Covered for the native contract on the public s3 × turso cell via opt-in `cohort_policy`. The object-log + SQLite pairing is not a current cell (ADR-024). | API-001, ADR-004, ADR-024, TD-010 |
| How recurring / never-terminal item lifecycle works: `rearm` outcome (caller-supplied eligibility materialized as `max(commit_time, not_before)` fed into the shared Eligibility Precedence timing stage, attempt-exempt, per-cycle retry reset), explicit termination (terminal finalize / `recurrence.until` drain / targeted in-band `PurgeItems` with tombstone + replay semantics, distinct from the P1 operator purge contract), recurring metrics served from the metrics projection (idle vs active, excluded from retry backlog/failure and from oldest eligible age while idle), queue-global (not per-group) progress parity, owner-local co-residency by construction (D2 / ADR-008), cohort / recurrence mutual exclusivity (ADR-004), gate composition, active-discovery parity via `fireweed_group_summary`, and queue-local purge replay semantics. | Covered for the native contract and both backends. | API-001, ADR-004, TD-001, TD-002, TD-004 |
| Whether the headline horizontal-scale claim is substantiated by a buildable, benchmarked mechanism rather than by `postgres_native` alone. | Covered by committed v1 per-queue ownership + cross-queue distribution (ADR-008) + object-log backend. E2/E3 must prove exact outcomes, queue-global progress, fencing, recovery, and bounded shared resources under load. Absolute rates remain capacity observations for the declared topology. | product-vision, PRD, ADR-001, ADR-008, TD-001, TD-003, TD-004, TP-002 |
| Whether a single node supports the required queue density (at least 1000 concurrently active cold queues plus one hot queue) without per-queue resource blow-up or cross-queue degradation. | Covered by the bounded shared per-node resource rule: per-queue background work (lease-expiry sweeps, progress-bound aggregation, summary recompute, recurring rearm, idempotency/retention GC) multiplexed onto bounded shared pools, batched per-node lease renewal, and LRU-bounded per-queue projection handles. TP-002 E2 requires the exact 1,001-queue shape, non-empty cold claims during sustained hot load, exact count reconciliation, and same-run controls. | PRD, ADR-002, ADR-003, TD-001, TD-002, TD-003, TD-004, TP-002 |
| How migration from Seventh Sense's existing queue-like tables preserves tenant/account boundaries and operational visibility. | Deferred until fireweed core and Postgres-native backend exist; requires a later migration design. | PRD, TD-001 |
