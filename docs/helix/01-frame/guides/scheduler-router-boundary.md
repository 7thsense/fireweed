...
fireweed owns durable queue state and queue-local leasing. Routers and schedulers
may decide where a worker should ask next and how much downstream work the
worker is allowed to start, but they must treat fireweed claims as the only
...
boundary: fireweed exposes queue-native pacing controls such as `max_items`,
`not_before`, group filters, gates, and finalize retry. It does not issue or
spend downstream API tokens.

## Responsibility Matrix

| Responsibility | fireweed | Router | Scheduler | Worker | Downstream | Stateless helper library |
|---|---|---|---|---|---|---|
...
| fireweed deployment and tenant capacity controls | Owns deployment-level rate limits, noisy-tenant quotas, and tenant capacity controls as PRD P1/API-001 `rate_limited`; this is currently unbuilt. | Surfaces fireweed `rate_limited` as fireweed capacity pressure, not downstream pressure. | Does not replace fireweed capacity controls. | Retries or backs off according to fireweed errors. | No role. | May classify fireweed errors for callers. |
...
descriptors, or map downstream outcomes to fireweed finalization requests. They
must not expose scheduler traits from fireweed, create token buckets in fireweed, or
persist routing state inside fireweed.
...
4. The worker calls fireweed claim. fireweed may return fewer than `max_items`, or
...
6. The worker finalizes every returned lease with complete, fail, retry, or
   ...
   work according to the normal fireweed contract.

`not_before` and finalize retry are queue eligibility mechanisms. Use them to
defer an item in fireweed after the caller has decided a later attempt is needed.
Do not treat them as downstream tokens. Gates and group filters are fireweed
...
- Do not access fireweed backends directly from a scheduler or router. Use the
...
- Do not interpret fireweed `rate_limited` as a downstream API rate result.
  API-001 reserves it for fireweed deployment or tenant capacity pressure.
- Do not add scheduler traits, downstream token buckets, quota registries, or
  persisted routing state to fireweed core.
