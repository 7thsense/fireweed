...
**Inventory ID:** `fireweed-1b1fb5ec-inventory`
...
| **Code location** | `crates/fireweed-objectlog/src/segmented.rs:1662` |
...
| **Code location** | `crates/fireweed-objectlog/src/segmented.rs` — `acquire_epoch()` at 1506, `commit_manifest_entry()` at 1096, `read_manifest_head()` at 134, `update_manifest_head_if_version()` at 163 |
...
| **Evidence** | `docs/helix/05-review/head-cas-linearizability-review.md:54` §Findings row F3 — review transcript documents `create_new(true)` non-atomicity on FUSE/overlay; `crates/fireweed-objectlog/src/segmented.rs:60-65` — `BlobStore::put_if_absent` trait contract; `LocalFsBlobStore` impl — `OpenOptions::new().create_new(true)` used for local-fs CAS |
| **Source** | `docs/helix/05-review/head-cas-linearizability-review.md` §Findings, row F3 |
| **Area** | BlobStore CAS contract portability |
| **Code location** | `crates/fireweed-objectlog/src/segmented.rs:60–65` (BlobStore::put_if_absent), `LocalFsBlobStore` impl (local-fs atomic create via `OpenOptions::new().create_new(true)`) |
...
| **Code location** | `crates/fireweed-objectlog/src/segmented.rs:1668–1684` (reclaim-time fence in seal()) |
| **Context** | `seal()` checks its in-memory `manifest_deletion_watermark` and self-fences if the target index was already known reclaimed. A concurrent trim can advance marker authority after that cache was loaded. The stale seal may therefore write one content-addressed segment before its create-only publication loses at the retained manifest-head/authority address. The segment is unreachable and reclaimable, and the operation returns `EpochFenced`; this is bounded correctness-preserving waste, not a data-safety hole. Avoiding it would add an object-store read/LIST to the successful steady-state seal path. |
...
| **Code location** | `crates/fireweed-objectlog/src/segmented.rs` — `commit_manifest_entry()` has no visible fallback path routing to Postgres |
...
| **Severity** | NOTE |
| **Disposition** | non-blocking |
| **Evidence** | `docs/helix/05-review/head-cas-linearizability-review.md:57` §Findings row F6 — review transcript documents unbounded retry budget pathology; `crates/fireweed-objectlog/src/segmented.rs:1506-1551` — `acquire_epoch()` bounded retry loop with 16-retry budget |
| **Source** | `docs/helix/05-review/head-cas-linearizability-review.md` §Findings, row F6 |
| **Area** | Epoch acquisition retry policy |
| **Code location** | `crates/fireweed-objectlog/src/segmented.rs:1506–1551` (acquire_epoch bounded retry loop) |
