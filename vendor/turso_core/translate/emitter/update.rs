...
fn emit_update_insns<'a>(
    ...
    for (index, (idx_cursor_id, record_reg)) in indexes_to_update.iter().zip(index_cursors) {
        ...
        // A row outside a partial index has no new key. Skip expression
        // evaluation and record construction as well as the constraint check;
        // Phase 3 uses this same predicate before reading the key registers.
        let skip_new_key = new_satisfies_where.map(|reg| {
        ...
        if let Some(label) = skip_new_key {
