...
async fn run_turso(path: &str) -> Result<(Vec<String>, usize, String), Box<dyn std::error::Error>> {
    ...
    tx.execute(
        "INSERT INTO pqueue_group_summary(tenant_id,queue_id,group_key,oldest_eligible_at,\
         ...
         at_risk_count,updated_at) VALUES('t','q','g',1,NULL,X'01',2,'item-b',2,0,2) \
    ...
    .await?;
    tx.execute(
        "UPDATE relational_cursor SET next_seq=2,next_item_seq=3 WHERE tenant='t' AND queue='q'",
        (),
    )
    .await?;
    tx.commit().await?;
    ...
    assert_eq!(eligible, ["item-b", "item-a"]);
    assert_eq!(
        text_rows(
            &conn,
    ...
    rollback
        .execute(
            "UPDATE relational_cursor SET next_seq=3 WHERE tenant='t' AND queue='q'",
            (),
        )
        .await?;
    ...
    assert_eq!(
        scalar_i64(&reopened_conn, "SELECT COUNT(*) FROM pqueue_items").await?,
        2
    );
    assert_eq!(
        scalar_i64(
    ...
    assert_eq!(text_rows(&reopened_conn, eligible_sql).await?, eligible);
    reopened_conn
        .execute_batch(
    ...
    println!("turso.batch_high_water_eligible_rollback_reopen=pass");
    ...
    Ok((eligible, attempts.len(), conflict))
}

fn run_rusqlite(path: &str) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    ...
    let eligible = {
        let mut stmt = conn.prepare(eligible_sql)?;
    ...
    let reopened_count: i64 =
        reopened.query_row("SELECT COUNT(*) FROM pqueue_items", [], |row| row.get(0))?;
    ...
    println!("rusqlite.schema_batch_high_water_eligible_rollback_reopen=pass");
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (turso_eligible, writers, _) = run_turso(turso_path.to_str().unwrap()).await?;
    let sqlite_eligible = run_rusqlite(sqlite_path.to_str().unwrap())?;
    assert_eq!(turso_eligible, sqlite_eligible);
    assert_eq!(writers, WRITER_COUNT);
    println!("comparison.eligible_order=equal:{turso_eligible:?}");
    println!("decision=no-go-current-adapter");
    println!(
        "reason=Turso SQL behavior is promising, but its async-only API cannot implement pqueue's \
         synchronous ProjectionStore atomic unit of work without a trait redesign or blocking actor."
    );
    Ok(())
}
