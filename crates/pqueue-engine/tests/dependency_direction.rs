//! Hexagonal dependency-direction gate (plan §1 + §6 DoD): the DOMAIN crates (`pqueue-core`,
//! `pqueue-engine`) must NOT depend on any adapter (driven or driving) or the composition root. Adapters
...
fn domain_crates_do_not_depend_on_adapters() {
    ...
    let core = include_str!("../../pqueue-core/Cargo.toml");

    for adapter in ADAPTERS {
        assert!(
            !depends_on(engine, adapter),
            "pqueue-engine must not depend on adapter `{adapter}` (hexagonal direction: adapters depend \
             on the domain, never the reverse)"
        );
        assert!(
            !depends_on(core, adapter),
            "pqueue-core must not depend on adapter `{adapter}`"
        );
    }
    // The innermost crate (core) must not depend even on the engine.
    assert!(
        !depends_on(core, "pqueue-engine"),
        "pqueue-core is the innermost domain crate and must depend on nothing pqueue-*"
    );
}
