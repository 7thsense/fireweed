...
mod item_mutation_tests {
    ...
    #[test]
    #[ignore = "manual CPU diagnostic; workflow qualification is authoritative"]
    fn compare_cached_and_uncached_record_columns() {
        use std::hint::black_box;
        use std::time::Instant;
        use turso_core::types::{ImmutableRecord, Value as CoreValue};
        for width in [8, 20] {
            let values: Vec<CoreValue> = (0..width)
                .map(|i| match i % 4 {
                    0 => CoreValue::Null,
                    1 => CoreValue::from_i64(1000 + i as i64),
                    2 => CoreValue::Text("metadata field".into()),
                    _ => CoreValue::Blob(vec![i as u8; 128]),
                })
                .collect();
            ...
            for cached in [false, true, true, false] {
                ...
                for _ in 0..100_000 {
                    for column in 0..width {
                        let value = if cached {
                            record.iter_at_column(black_box(column)).unwrap().next().unwrap().unwrap()
                        } else {
                            record.iter().unwrap().nth(black_box(column)).unwrap().unwrap()
                        };
                        black_box(value);
                    }
                }
                println!(
                    "record_columns width={width} cached={cached} elapsed_s={:.6} record_bytes={}",
    ...
    #[test]
    fn record_column_position_cache_matches_uncached_reads_and_mutations() {
        use turso_core::types::{ImmutableRecord, Value as CoreValue};

        fn check(record: &ImmutableRecord, column: usize) {
            let expected = record.iter().unwrap().nth(column).transpose().unwrap();
            let actual = record
                .iter_at_column(column)
                .unwrap()
                .next()
                .transpose()
                .unwrap();
        ...
        let values: Vec<CoreValue> = (0..160)
            .map(|i| match i % 6 {
                0 => CoreValue::Null,
                1 => CoreValue::from_i64(i64::MIN + i),
                2 => CoreValue::from_i64(i % 2),
                3 => CoreValue::Blob(vec![i as u8; 16384 + i as usize]),
                4 => CoreValue::Text("a varied text column".into()),
                _ => CoreValue::from_f64(i as f64 + 0.5),
            })
            .collect();
        let mut record = ImmutableRecord::from_values(&values, values.len()).unwrap();
        for column in 0..values.len() + 4 {
            check(&record, column);
        }
        for column in (0..values.len()).rev() {
            check(&record, column);
        }
        let mut rng = 71237u64;
        for _ in 0..2000 {
            rng ^= rng << 13;
            rng ^= rng >> 7;
            rng ^= rng << 17;
            check(&record, rng as usize % (values.len() + 3));
        }
        let cloned = record.clone();
        check(&cloned, 117);
        // Every public mutable-payload path invalidates offsets, even when
        // the vector's allocation is retained and its address does not change.
        let replacement = ImmutableRecord::from_values(
            &[CoreValue::Text("new".into()), CoreValue::from_i64(42)],
            2,
        )
        .unwrap();
        let bytes = replacement.get_payload().to_vec();
        record.as_blob_mut().clear();
        record.as_blob_mut().extend_from_slice(&bytes);
        check(&record, 1);
        check(&record, 159);
        check(&record, 0);
        record.invalidate();
        record.start_serialization(cloned.get_payload()).unwrap();
        check(&record, 159);
        check(&record, 1);
        // Both offsets are published together; readers may replace each
        // other's cache entry without affecting the underlying record.
        std::thread::scope(|scope| {
            for offset in 0..4 {
                let record = &record;
                scope.spawn(move || {
                    for i in 0..400 {
                        check(record, (i * 37 + offset) % 160);
                    }
                });
            }
        });
        // Header positions / column ordinals outside the packed cache's
        // bounds remain valid reads through the uncached fallback.
        let wide = vec![CoreValue::Null; 65540];
        let wide_record = ImmutableRecord::from_values(&wide, wide.len()).unwrap();
        for column in [1, 65534, 65535, 65536, 65539, 65540, 2] {
            check(&wide_record, column);
        }
    }

    #[test]
    fn record_column_position_cache_preserves_corruption_errors() {
        use turso_core::types::{ImmutableRecord, Value as CoreValue};
        let values = [
            CoreValue::from_i64(42),
            CoreValue::Blob(vec![5; 80]),
            CoreValue::from_i64(7),
        ];
        let valid = ImmutableRecord::from_values(&values, values.len()).unwrap();
        for cut in 0..valid.get_payload().len() {
            let record = ImmutableRecord::from_bin_record(valid.get_payload()[..cut].to_vec());
            for column in [0, 1, 2, 1, 0, 2] {
                let expected = record.iter().and_then(|mut it| it.nth(column).transpose());
                let actual = record
                    .iter_at_column(column)
        ...
        // Reserved serial types must still error after warming the cache.
        let record = ImmutableRecord::from_bin_record(vec![5, 8, 9, 10, 8]);
        assert!(record.iter_at_column(1).unwrap().next().unwrap().is_ok());
        assert!(record.iter_at_column(3).is_err());
    }
