...
//!   * the generic throughput workloads (`ingest`, `claim_ack`) over the [`pqueue::LibBackend`] facade;
...
pub async fn ingest<B: pqueue::LibBackend>(
    pq: &Pqueue<B>,
    q: &QueueKey,
    shape: &Shape,
    items: u64,
...
pub async fn claim_ack<B: pqueue::LibBackend>(
    pq: &Pqueue<B>,
    q: &QueueKey,
    items: u64,
    batch: usize,
...
async fn claim_n<B: pqueue::LibBackend>(
    pq: &Pqueue<B>,
    q: &QueueKey,
    target: u64,
    batch: usize,
...
/// reclaim + re-drain.
pub async fn lifecycle<B: pqueue::LibBackend>(
    pq: &Pqueue<B>,
    q: &QueueKey,
    shape: &Shape,
    items: u64,
