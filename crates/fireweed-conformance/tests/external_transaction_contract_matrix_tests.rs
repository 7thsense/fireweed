...
//! fault injection, using the reusable [`fireweed_conformance::fault`] harness.
//!
...
//! instants drive [`fireweed_objectlog::ObjectLog`]'s `LogStore` impl DIRECTLY (bypassing `ComposedBackend`)
//! with the [`fireweed_objectlog::FaultHook`] seam, striking cut points strictly INSIDE the segmented
...
//! projection apply before response" — live one layer up in `fireweed-engine`'s `ComposedBackend` (which applies
//! a batch only after `LogStore::append` returned `Ok`), so they need the engine's [`fireweed_engine::ComposeFaultPoint`]
...
//! [`fireweed_sqlite::HybridFaultHook`] on `HybridProjectionStore` — for the instants the public seam cannot
...
use fireweed_core::{ClientItemKey, ItemId, RequestId};
use fireweed_engine::{
...
fn sqlite_log_factory() -> impl Fn(&str) -> fireweed_sqlite::ComposedSqliteBackend {
...
fn sqlite_relational_factory() -> impl Fn(&str) -> fireweed_sqlite::ComposedSqliteRelationalBackend
{
    let base = base_dir("sqlite-relational");
    move |tag: &str| {
        std::fs::create_dir_all(&base).ok();
        let path = base.join(format!("{tag}.db"));
        fireweed_sqlite::composed_sqlite_relational(path.to_str().unwrap())
            .expect("open composed sqlite-relational")
    }
}

fn objectlog_factory() -> impl Fn(&str) -> fireweed_objectlog::ComposedObjectLogBackend {
...
fn objectlog_group_commit_factory() -> impl Fn(&str) -> fireweed_objectlog::ComposedObjectLogBackend
{
    let base = base_dir("objectlog-gc");
    move |tag: &str| {
        fireweed_objectlog::composed_objectlog_backend_group_commit(
            base.join(tag),
...
fn objectlog_factory_at(
    bound_ms: u64,
) -> impl Fn(&str) -> fireweed_objectlog::ComposedObjectLogBackend {
...
/// The two Postgres-log storage pairs exposed by `fireweed-server`.  These are deliberately assembled from
...
fn postgres_sqlite_factory(
    ...
    move |tag: &str| {
        ...
        let log = fireweed_postgres::PostgresLog::connect_in_schema(&url, &log_schema)
            .expect("connect postgres/sqlite log axis");
        let projection =
            fireweed_sqlite::SqliteProjectionStore::open(projection_path.to_str().unwrap())
...
fn postgres_postgres_factory(
    ...
    move |tag: &str| {
        ...
        let projection =
            fireweed_postgres::PostgresRelational::connect_in_schema(&url, &projection_schema)
...
// ---------------------------------------------------------------------------
//
// `fireweed_conformance` deliberately depends only on the domain (engine + core) — adapters depend on IT,
// not the reverse (see the crate doc) — so this scenario cannot live in `fireweed_conformance::fault`
// alongside the AC-TXN-1..3/6 scenarios; it lives here, in the objectlog-specific test binary, which
// already carries `fireweed_objectlog` as a dev-dependency. It drives `ObjectLog`'s `LogStore` impl DIRECTLY
...
impl ComposeFaultHook for ComposeCrashAt {
    fn fault_point(&self, cut: ComposeFaultPoint) -> fireweed_engine::EngineResult<()> {
        if cut == self.0 {
            Err(EngineError::Storage(format!(
...
fn ac_txn_4_push_env(id: &str, key: &str) -> fireweed_engine::CommandEnvelope {
    fireweed_conformance::envelope(
        fireweed_engine::QueueCommand::Push(fireweed_engine::PushCommand {
            items: vec![fireweed_conformance::item(id, key, 5)],
        }),
        vec![],
    )
}
...
async fn ac_txn_4_crash_point_matrix() -> AcOutcome {
    ...
    // way must not resurrect as two leases (0 duplicate active leases).
    {
        ...
        let claim_env = fireweed_conformance::envelope(
            fireweed_engine::QueueCommand::Claim(ClaimCommand {
                item_ids: vec![fireweed_core::ItemId::new("1").unwrap()],
                lease_token: fireweed_core::LeaseToken::new("lease-1").unwrap(),
                lease_expires_at: fireweed_conformance::ts(500),
                worker_id: None,
            }),
            vec![fireweed_core::ItemId::new("1").unwrap()],
        );
        ...
        let claim_entries = all_entries
            .iter()
            .filter(|(_, env)| matches!(env.command, fireweed_engine::QueueCommand::Claim(_)))
            .count();
        ensure!(
            claim_entries == 1,
    ...
    // substrate the 5 cuts above drive directly; they live one layer up, in the `fireweed-engine`
...
/// instants row 209 names live in the `fireweed-engine` [`ComposedBackend`] apply step, ABOVE the segmented
...
async fn ac_txn_4_composed_projection_apply_crash(cut: ComposeFaultPoint) -> AcOutcome {
    ...
    let shard = fireweed_conformance::shard();
    let qkey = fireweed_conformance::qkey();
    ...
    let acked = {
        let a = open()?;
        a.create_queue(fireweed_conformance::qdef())
            .await
        ...
        let acked = a
            .push_with_request_id(
                ...
                rid.clone(),
                vec![
                    fireweed_conformance::fault::spec("txn4c-a1", 9),
                    fireweed_conformance::fault::spec("txn4c-a2", 5),
                    fireweed_conformance::fault::spec("txn4c-a3", 1),
                ],
        ...
        let batch = vec![
            fireweed_conformance::envelope(
                QueueCommand::Claim(ClaimCommand {
                    item_ids: vec![a1],
                    lease_token: fireweed_core::LeaseToken::new("compose-lease-1").unwrap(),
                    lease_expires_at: fireweed_conformance::ts(500),
                    worker_id: None,
                }),
                vec![a1],
    ...
    // RECONSTRUCTED PROJECTED STATE.
    {
        ...
        let reclaim = c
            .claim(fireweed_conformance::claim_req(10, 1500, 700))
            .await
...
async fn ac_txn_1_kill_after_create_queue() {
    let sr =
        fireweed_conformance::fault::ac_txn_1_kill_after_create_queue(sqlite_relational_factory())
            .await;
    assert!(sr.is_ok(), "sqlite_relational: {:?}", sr.err());
    let sq =
        fireweed_conformance::fault::ac_txn_1_kill_after_create_queue(sqlite_log_factory()).await;
    assert!(sq.is_ok(), "sqlite_log: {:?}", sq.err());
    let ol =
        fireweed_conformance::fault::ac_txn_1_kill_after_create_queue(objectlog_factory()).await;
    assert!(ol.is_ok(), "objectlog: {:?}", ol.err());
    let ols =
        fireweed_conformance::fault::ac_txn_1_kill_after_create_queue(objectlog_sqlite_factory())
            .await;
...
async fn ac_txn_1_kill_after_batch_update() {
    ...
    for (name, outcome) in [
        (
            "sqlite_relational",
            fireweed_conformance::fault::ac_txn_1_kill_after_batch_update(
...
async fn ac_txn_1_kill_after_set_gates() {
    ...
    for (name, outcome) in [
        ...
        (
            "objectlog",
            fireweed_conformance::fault::ac_txn_1_kill_after_set_gates(objectlog_factory()).await,
        ),
        (
            "object_log_sqlite",
            fireweed_conformance::fault::ac_txn_1_kill_after_set_gates(objectlog_sqlite_factory())
                .await,
        ),
    ] {
        assert!(outcome.is_ok(), "{name}: {:?}", outcome.err());
...
async fn ac_txn_1_kill_after_purge_items() {
    let sr =
        fireweed_conformance::fault::ac_txn_1_kill_after_purge_items(sqlite_relational_factory())
            .await;
    assert!(sr.is_ok(), "sqlite_relational: {:?}", sr.err());
    let sq =
        fireweed_conformance::fault::ac_txn_1_kill_after_purge_items(sqlite_log_factory()).await;
    assert!(sq.is_ok(), "sqlite_log: {:?}", sq.err());
    let ol =
        fireweed_conformance::fault::ac_txn_1_kill_after_purge_items(objectlog_factory()).await;
    assert!(ol.is_ok(), "objectlog: {:?}", ol.err());
    let ols =
        fireweed_conformance::fault::ac_txn_1_kill_after_purge_items(objectlog_sqlite_factory())
            .await;
    assert!(ols.is_ok(), "object_log_sqlite: {:?}", ols.err());
}
...
async fn ac_txn_2_envelope_invalid_batch_has_no_durable_effect() {
    let sr = fireweed_conformance::fault::ac_txn_2_envelope_invalid_batch(
        sqlite_relational_factory(),
        DURABLE,
    )
    .await;
    assert!(sr.is_ok(), "sqlite_relational: {:?}", sr.err());
    let sq =
        fireweed_conformance::fault::ac_txn_2_envelope_invalid_batch(sqlite_log_factory(), DURABLE)
            .await;
    assert!(sq.is_ok(), "sqlite_log: {:?}", sq.err());
    let ol =
        fireweed_conformance::fault::ac_txn_2_envelope_invalid_batch(objectlog_factory(), DURABLE)
            .await;
    assert!(ol.is_ok(), "objectlog: {:?}", ol.err());
    let ols = fireweed_conformance::fault::ac_txn_2_envelope_invalid_batch(
        objectlog_sqlite_factory(),
        DURABLE,
    )
    .await;
...
async fn ac_txn_2_stale_lease_conflict_has_no_durable_effect() {
    let sr = fireweed_conformance::fault::ac_txn_2_stale_lease_conflict(
        sqlite_relational_factory(),
        DURABLE,
    )
    .await;
    assert!(sr.is_ok(), "sqlite_relational: {:?}", sr.err());
    let sq =
        fireweed_conformance::fault::ac_txn_2_stale_lease_conflict(sqlite_log_factory(), DURABLE)
            .await;
    assert!(sq.is_ok(), "sqlite_log: {:?}", sq.err());
    let ol =
        fireweed_conformance::fault::ac_txn_2_stale_lease_conflict(objectlog_factory(), DURABLE)
            .await;
    assert!(ol.is_ok(), "objectlog: {:?}", ol.err());
    let ols = fireweed_conformance::fault::ac_txn_2_stale_lease_conflict(
        objectlog_sqlite_factory(),
        DURABLE,
    )
    .await;
...
async fn ac_txn_2_commit_timeout_path_has_no_durable_effect() {
    let sr = fireweed_conformance::fault::ac_txn_2_commit_timeout_path(
        sqlite_relational_factory(),
        DURABLE,
    )
    .await;
    assert!(sr.is_ok(), "sqlite_relational: {:?}", sr.err());
    let sq =
        fireweed_conformance::fault::ac_txn_2_commit_timeout_path(sqlite_log_factory(), DURABLE)
            .await;
    assert!(sq.is_ok(), "sqlite_log: {:?}", sq.err());
    let ol =
        fireweed_conformance::fault::ac_txn_2_commit_timeout_path(objectlog_factory(), DURABLE)
            .await;
    assert!(ol.is_ok(), "objectlog: {:?}", ol.err());
    let ols = fireweed_conformance::fault::ac_txn_2_commit_timeout_path(
        objectlog_sqlite_factory(),
        DURABLE,
    )
    .await;
...
async fn ac_txn_3_request_id_replay_all_ops_all_cut_points() {
    ...
    let mem_ct = ac_txn_3_commit_transition_request_id(
        |_: &str| fireweed_memory::composed_memory_backend(),
        NON_DURABLE,
    )
    .await;
...
// (`fireweed_objectlog::segmented`, AC-TXN-4); this section adds the analogous seam on the PROJECTION side —
// `fireweed_sqlite::HybridFaultHook` — striking instants strictly INSIDE `HybridProjectionStore`'s own apply
...
fn hybrid_push_env(id: &str, key: &str) -> fireweed_engine::CommandEnvelope {
    fireweed_conformance::envelope(
...
async fn ac_txn_5_hybrid_strict_poison_replay_scenario() -> AcOutcome {
    ...
    // observes it. The store must poison and fail every subsequent op closed, before any restart. ---
    {
        ...
        ProjectionStore::ensure_shard(&mut hybrid, &fireweed_conformance::qdef())
            .map_err(|e| format!("ensure_shard: {e:?}"))?;
    ...
    // detection), independent of the poisoned instance above. ---
    {
        ...
        let body = vec![fireweed_conformance::fault::spec("ac-txn-5-a", 1)];
        let first = backend
            .push_with_request_id(
                &shard,
                rid.clone(),
        ...
        let replay = backend
            .push_with_request_id(&shard, rid.clone(), body, fireweed_conformance::ts(2), None)
            .await
        ...
        let conflict = backend
            .push_with_request_id(
                &shard,
                rid,
                vec![fireweed_conformance::fault::spec("ac-txn-5-different", 2)],
...
async fn strict_push(
    ...
    backend
        .push_with_request_id(
            &fireweed_conformance::shard(),
            rid,
            vec![fireweed_conformance::fault::spec(key, 5)],
            fireweed_conformance::ts(1),
            None,
        )
        .await
...
async fn ac_txn_5_hybrid_strict_poison_on_real_server_path_scenario() -> AcOutcome {
    ...
    // item id (0 duplicate transitions) and a conflicting body must return request-id-conflict. ---
    {
        ...
        let body = vec![fireweed_conformance::fault::spec("rid-item", 7)];
        ...
        // durably-committed-but-unreturned item is present exactly once.
        {
            ...
            let conflict = backend
                .push_with_request_id(
                    &shard,
                    rid,
                    vec![fireweed_conformance::fault::spec("rid-item-different", 8)],
                    fireweed_conformance::ts(3),
                    None,
                )
                .await;
...
async fn ac_txn_5a_retention_advancement_scenario(target_bytes: usize, bound_ms: u64) -> AcOutcome {
    ...
    // reclaimable at `now`.
    let reap =
        |b: &HybridBackend| b.reap_terminal_items(&shard, fireweed_conformance::ts(1), 0, false);
    ...
    let claimed = backend
        .claim(fireweed_conformance::claim_req(10, 5_000, 1))
        .await
    ...
    let outcomes = claimed
        ...
        .collect::<Vec<_>>();
    backend
        .finalize(&shard, outcomes, fireweed_conformance::ts(1), None)
        .await
...
async fn ac_txn_5a_retention_no_id_resurrection_scenario(
    ...
    let reaped = backend
        .reap_terminal_items(&shard, fireweed_conformance::ts(1), 0, false)
...
async fn ac_txn_5a_hybrid_async_success_barrier_scenario() -> AcOutcome {
    ...
    // manifest commit that preceded it IS durable. ---
    {
        ...
        let body = vec![fireweed_conformance::fault::spec(
            "ac-txn-5a-barrier-item",
            5,
        )];
        let err = backend
            .push_with_request_id(&shard, rid, body, fireweed_conformance::ts(1), None)
            .await;
        ensure!(
            err.is_err(),
    ...
    // in order, and the SQLite logical high-water advances through the whole batch exactly once. ---
    {
        ...
        let batch: Vec<(CommandPosition, fireweed_engine::CommandEnvelope)> = (0..3)
            .map(|i: u64| {
    ...
    // CLOSED): the `hybrid-async` arm of `fireweed-server` now arms the `HybridAsyncMonitor` inside the
...
async fn ac_txn_5a_segment_object_reclamation_scenario() -> AcOutcome {
    ...
    let floor_seq = |b: &HybridBackend| {
        b.with_log(|l| LogStore::retention_floor(l, &fireweed_conformance::shard()))
            .ok()
            .flatten()
            .map(|p| p.sequence)
    };
    ...
    backend
        .push(
            &shard,
            vec![fireweed_conformance::fault::spec("filler", 5)],
            fireweed_conformance::ts(1),
            None,
        )
        .await
    ...
    let r_ids = backend
        .push_with_request_id(
            ...
            vec![fireweed_conformance::fault::spec("R-body", 5)],
            fireweed_conformance::ts(1_000),
            None,
        )
        .await
    ...
    let report = backend
        .tick(fireweed_conformance::ts(4_000))
        .await
    ...
    let r2_ids = backend2
        .push_with_request_id(
            ...
            vec![fireweed_conformance::fault::spec("R2-body", 5)],
            fireweed_conformance::ts(1_000),
            None,
        )
        .await
    ...
    let report_b = backend2
        .tick(fireweed_conformance::ts(10_000_000))
        .await
    ...
    let fresh = reopened2
        .push_with_request_id(
            &shard,
            RequestId::new("R2".to_string()).unwrap(),
            vec![fireweed_conformance::fault::spec("R2-fresh-body", 5)],
            fireweed_conformance::ts(10_000_000),
            None,
        )
        .await
...
async fn ac_txn_7_numeric_latency_sweep() -> AcOutcome {
    use fireweed_engine::{ControlPlaneStore, ProjectionRead, PushPort};
    ...
    async fn run_bound(bound_ms: u64) -> Result<(Vec<String>, String), String> {
        ...
        let cobuf_ids = backend
            .push(
                &shard,
                vec![fireweed_conformance::fault::spec("cobuf", 5)],
                fireweed_conformance::ts(1),
                None,
            )
            .await
        ...
        let eligible_after_push = backend
            .select_eligible(&shard, fireweed_conformance::ts(100), 10)
            .await
        ...
        let body = vec![fireweed_conformance::fault::spec("ridbody", 7)];
        let orig = backend
            .push_with_request_id(
                &shard,
                rid.clone(),
        ...
        let replay = backend
            .push_with_request_id(&shard, rid.clone(), body, fireweed_conformance::ts(1), None)
            .await
        ...
        let conflict = backend
            .push_with_request_id(
                &shard,
                rid,
                vec![fireweed_conformance::fault::spec("ridbody-DIFFERENT", 8)],
                fireweed_conformance::ts(1),
                None,
            )
            .await;
        ...
        let lost_body = vec![fireweed_conformance::fault::spec("lost", 3)];
        ...
        let replay2 = backend2
            .push_with_request_id(&shard, rid2, lost_body, fireweed_conformance::ts(2), None)
            .await
        ...
        let eligible = backend2
            .select_eligible(&shard, fireweed_conformance::ts(100), 100)
            .await
    ...
    // its synchronous buffer+register prologue — and must observe it still Pending.
    {
        ...
        let root = base.join("run");
        std::fs::create_dir_all(&root).ok();
        let backend = fireweed_objectlog::composed_objectlog_backend_group_commit(
            &root,
        ...
        let parked = backend
            .push(
                &shard,
                vec![fireweed_conformance::fault::spec("noflush", 5)],
                fireweed_conformance::ts(1),
                None,
            )
            .now_or_never();
    ...
    async fn run_bound(bound_ms: u64) -> Result<(Vec<String>, Vec<String>, String), String> {
        ...
        // co-buffering path AT this bound. ---
        {
            ...
            let ids = backend
                .push(
                    &shard,
                    vec![fireweed_conformance::fault::spec("live", 5)],
                    fireweed_conformance::ts(1),
                    None,
                )
                .await
        ...
        // failure withholds success even though the preceding object-log manifest commit is durable. ---
        {
            ...
            let body = vec![fireweed_conformance::fault::spec("barrier-item", 5)];
            let err = backend
                .push_with_request_id(&shard, rid, body, fireweed_conformance::ts(1), None)
                .await;
        ...
        // fails CLOSED (Unavailable) with 0 durable + 0 projected effect. ---
        {
            ...
            for i in 0..5u64 {
                backend
                    .push(
                        &shard,
                        vec![fireweed_conformance::fault::spec(&format!("hd-{i}"), 5)],
                        fireweed_conformance::ts(1),
                        None,
                    )
                    .await
            ...
            let rejected = backend
                .push(
                    &shard,
                    vec![fireweed_conformance::fault::spec("hd-rejected", 5)],
                    fireweed_conformance::ts(1),
                    None,
                )
                .await;
        ...
        // batch queued (0 dropped) + poisons the store fail-closed. ---
        {
            ...
            for i in 0..3u64 {
                backend
                    .push(
                        &shard,
                        vec![fireweed_conformance::fault::spec(&format!("drain-{i}"), 5)],
                        fireweed_conformance::ts(1),
                        None,
                    )
                    .await
        ...
        }
        {
            ...
            backend
                .push(
                    &shard,
                    vec![fireweed_conformance::fault::spec("chk", 5)],
                    fireweed_conformance::ts(1),
                    None,
                )
                .await
    ...
    FF: Fn() -> Result<ComposedBackend<ObjectLog, P, C>, String>,
{
    ...
    let latency_ids = latency
        .push(
            &shard,
            vec![fireweed_conformance::fault::spec("e3-ac7-window", 9)],
            fireweed_conformance::ts(1),
            None,
        )
        .await
    ...
    let latency_body = vec![fireweed_conformance::fault::spec("e3-ac7-rid", 5)];
    ...
    let force_rid = RequestId::new(format!("e3-ac7-force-{profile}-{bound_ms}"))
        .map_err(|e| format!("request id: {e:?}"))?;
    let force_body = vec![fireweed_conformance::fault::spec("e3-ac7-rid", 5)];
...
async fn e3_ac_txn_7_cell(profile: &str, bound_ms: u64) -> AcOutcome {
    ...
    if profile == "object_log_inmemory_projection" {
        e3_ac_txn_7_exact_cell(
            ...
            || {
                fireweed_objectlog::composed_objectlog_backend(&force_root)
...
fn ac_txn_contract_matrix_postgres() {
    ...
    let pg_factory = |prefix: String, url: String| {
        ...
        move |tag: &str| {
            ...
            fireweed_postgres::composed_postgres_backend_in_schema(&url, &sch)
...
/// `fireweed-server`.  Unlike `ac_txn_contract_matrix_postgres`, neither row substitutes an in-memory
