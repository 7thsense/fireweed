...
use fireweed_conformance::{qdef, qkey, ts};
use fireweed_engine::{
...
fn floor_pos(backend: &HybridBackend) -> Option<fireweed_engine::CommandPosition> {
    backend
        .with_log(|l| LogStore::retention_floor(l, &qkey()))
        .expect("retention_floor")
}
            ...
            .push(
                ...
                vec![PushSpec {
                    client_item_key: Some(
                        fireweed_core::ClientItemKey::new(format!("{prefix}-{i}")).unwrap(),
                    ),
...
fn branch_qkey() -> fireweed_engine::QueueKey {
    fireweed_engine::QueueKey::new(
        qkey().tenant_id.clone(),
        fireweed_core::QueueId::new("branch-rfsp-engine").unwrap(),
    )
}

fn branch_def() -> fireweed_core::QueueDefinition {
    let mut d = qdef();
...
async fn retention_floor_fail_closed_and_recovery_impl(root: &Path, mode: ProjectionMode) {
    ...
    let from_zero_err = backend
        .read_from(
            &qkey(),
            Some(fireweed_engine::CommandPosition::new(qkey(), 0, 0)),
            100,
        )
        .await
...
async fn source_pin_blocks_reclamation_impl(root: &Path, mode: ProjectionMode) {
    ...
    let pin_pos = fireweed_engine::CommandPosition::new(qkey(), 0, 0);
    backend
