// Shared async projection test-fixture support for integration tests.
//
...
// live in the calling test file (which adds `pub use fireweed_conformance::{qdef, ts}`
// and has its own `fn shard`), so `super::` resolves correctly.

include!("../../src/async_projection_compat/mod.rs");
