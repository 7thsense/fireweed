...
// `pg_conformance!` macro in crates/fireweed-postgres/tests/conformance.rs) can reach the capability-gated
...
fn commit_transition_capable(caps: &fireweed_engine::CommitCapabilities) -> bool {
    caps.atomic_transition_commit
        ...
        && matches!(
            ...
            fireweed_engine::DurabilityClass::Atomic
        )
}
...
async fn commit_transition_declines_consistently<B: ConformanceCommitTransition>(
    ...
    assert_eq!(
        ...
        fireweed_engine::CommitCapabilities::default()
    );

    let err = b
        .commit_transition(
            &shard(),
            fireweed_engine::CommitTransition {
                request_id: None,
                entries: vec![],
            },
            ts(0),
...
) -> fireweed_engine::CommitTransitionEntry {
    fireweed_engine::CommitTransitionEntry {
        claim_ref,
        ...
        lifecycle_items: vec![PushSpec {
            priority: Some(fireweed_core::PriorityValue::Int64(lifecycle_priority)),
            ..Default::default()
        }],
        instance_fence,
    }
...
pub async fn commit_transition_rejects_bad_token_without_writing<B: ConformanceCommitTransition>(
    ...
    assert_eq!(
        outcomes,
        vec![fireweed_engine::CommitEntryOutcome::Rejected(
...
>(
    ...
    let body = |claim_ref: ClaimRef| fireweed_engine::CommitTransition {
        request_id: Some(rid.clone()),
        entries: vec![commit_transition_entry(
            claim_ref,
            FinalizeKind::Complete,
...
pub async fn claim_with_explicit_eligibility_time<B: ConformanceCore>(make: impl Fn() -> B) {
    ...
    fn ids(claimed: &fireweed_engine::Claimed) -> Vec<String> {
        claimed
            .items
            .iter()
            .map(|i| i.item_id.to_string())
...
pub async fn cross_family_core_parity<A: ConformanceCore, B: ConformanceCore>(
    ...
    async fn parity<A: ConformanceCore, B: ConformanceCore>(a: &A, b: &B, now: i64, label: &str) {
        ...
        let sort_pending = |v: Vec<fireweed_engine::LeaseView>| {
...
mod commit_transition_scenario_tests {
    ...
    fn temp_db_path(tag: &str) -> PathBuf {
        std::env::temp_dir().join(format!(
            "fireweed-conformance-commit-transition-{tag}-{}.db",
            std::process::id()
        ))
    }
