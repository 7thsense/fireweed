...
fn queue_density_single_node_tests() {
    ...
    let row = fireweed_release::LedgerRow {
        ...
        command: "cargo test --manifest-path crates/fireweed-bench/Cargo.toml --test queue_density_single_node_tests".into(),
...
fn queue_density_single_node_durable_tests() {
    ...
    fireweed_release::append_row(&evidence_path, &ol_row)
    ...
    if !pg_points.is_empty() {
        ...
        fireweed_release::append_row(&evidence_path, &pg_row)
            .expect("emit durable postgres density row");
    }
...
fn durable_density_row(
    ...
    let mut values = std::collections::BTreeMap::from([
        ...
        ("max_density".into(), serde_json::json!(top.density)),
        (
