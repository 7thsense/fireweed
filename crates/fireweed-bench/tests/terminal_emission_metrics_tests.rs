...
fn resident_terminal_count_tracks_terminal_population_via_metrics() {
    ...
    futures::executor::block_on(async {
        fireweed
            .create_queue(bench_qdef("bench", "terminal-metrics", &shape))
            .await
            .unwrap();

        fireweed
            .push_batch(&queue, vec![NewItem::default(), NewItem::default()])
            .await
            .unwrap();
        ...
        fireweed
            .ack(&queue, vec![claimed[0].item_id])
            .await
            .unwrap();
        fireweed
            .fail(&queue, vec![claimed[1].item_id])
            .await
            .unwrap();
