...
/// Projection-only encoding. Public values and the authoritative log retain their serde format.
/// Existing projection rows remain readable without a rewrite.
pub fn priority_to_json(value: &PriorityValue) -> EngineResult<String> {
    match value {
        PriorityValue::Timestamp(t) => to_json(&("t", t.seconds, t.nanoseconds)),
        PriorityValue::Int64(i) => to_json(&("i", i)),
        PriorityValue::Decimal(d) => to_json(&("d", d.mantissa, d.scale)),
        PriorityValue::Text(s) => to_json(&("s", s)),
    }
}

struct PriorityTag(u8);
impl<'de> serde::Deserialize<'de> for PriorityTag {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        struct TagVisitor;
        impl serde::de::Visitor<'_> for TagVisitor {
            type Value = PriorityTag;
            fn expecting(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
                f.write_str("a priority tag: t, i, d, or s")
            }
            fn visit_str<E: serde::de::Error>(self, tag: &str) -> Result<Self::Value, E> {
                match tag {
                    "t" | "i" | "d" | "s" => Ok(PriorityTag(tag.as_bytes()[0])),
                    _ => Err(E::custom("unknown compact priority tag")),
                }
            }
        }
...
struct StoredPriority(PriorityValue);
impl<'de> serde::Deserialize<'de> for StoredPriority {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        struct PriorityVisitor;
        impl<'de> serde::de::Visitor<'de> for PriorityVisitor {
            type Value = StoredPriority;
            fn expecting(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
                f.write_str("a compact priority array")
            }
            fn visit_seq<A: serde::de::SeqAccess<'de>>(
                self,
                mut seq: A,
            ) -> Result<Self::Value, A::Error> {
                fn required<'de, A: serde::de::SeqAccess<'de>, T: serde::Deserialize<'de>>(
                    seq: &mut A,
                ) -> Result<T, A::Error> {
                    seq.next_element()?
                        .ok_or_else(|| serde::de::Error::custom("missing priority field"))
                }
                let tag: PriorityTag = required(&mut seq)?;
                // Decode typed numbers directly: intermediate JSON values cannot preserve every i128.
                let priority = match tag.0 {
                    b't' => PriorityValue::Timestamp(UtcTimestamp {
                        seconds: required(&mut seq)?,
                        nanoseconds: required(&mut seq)?,
                    }),
                    b'i' => PriorityValue::Int64(required(&mut seq)?),
                    b'd' => PriorityValue::Decimal(fireweed_core::DecimalValue {
                        mantissa: required(&mut seq)?,
                        scale: required(&mut seq)?,
                    }),
                    b's' => PriorityValue::Text(required(&mut seq)?),
                    _ => unreachable!("PriorityTag validates tags"),
                };
                if seq.next_element::<serde::de::IgnoredAny>()?.is_some() {
                    return Err(serde::de::Error::custom("extra priority field"));
                }
                Ok(StoredPriority(priority))
            }
        }
...
pub fn parse_priority_ref(raw: Option<&str>) -> EngineResult<Option<PriorityValue>> {
    raw.map(|s| {
        let decoded = if s.trim_start().starts_with('[') {
            serde_json::from_str::<StoredPriority>(s).map(|stored| stored.0)
        } else {
            serde_json::from_str(s)
        };
        decoded.map_err(|e| EngineError::Storage(e.to_string()))
    })
    .transpose()
}

pub fn parse_priority(raw: Option<String>) -> EngineResult<Option<PriorityValue>> {
    parse_priority_ref(raw.as_deref())
}

#[cfg(test)]
mod compact_priority_contract_tests {
    use super::*;
    use fireweed_core::DecimalValue;

    #[test]
    fn legacy_and_compact_priorities_preserve_exact_values() {
        let cases = [
            (
                PriorityValue::Timestamp(UtcTimestamp::new(-1, 999_999_999).unwrap()),
                r#"["t",-1,999999999]"#.to_string(),
            ),
            (
                PriorityValue::Timestamp(UtcTimestamp::new(i64::MIN, 0).unwrap()),
                format!(r#"["t",{},0]"#, i64::MIN),
            ),
            (
                PriorityValue::Int64(i64::MIN),
                format!(r#"["i",{}]"#, i64::MIN),
            ),
            (
                PriorityValue::Int64(i64::MAX),
                format!(r#"["i",{}]"#, i64::MAX),
            ),
            (
            ...
            (PriorityValue::Text("".into()), r#"["s",""]"#.to_string()),
            (
                PriorityValue::Text("red\n\"blue\" 🦊".into()),
                r#"["s","red\n\"blue\" 🦊"]"#.to_string(),
            ),
        ];
        for (expected, compact) in cases {
            assert_eq!(priority_to_json(&expected).unwrap(), compact);
            let legacy = to_json(&expected).unwrap();
            assert_eq!(
                parse_priority(Some(legacy.clone())).unwrap(),
                Some(expected.clone()),
                "{legacy}"
            );
            assert_eq!(
                parse_priority(Some(compact.clone())).unwrap(),
                Some(expected),
                "{compact}"
            );
        }
        ...
        assert_eq!(
            parse_priority(Some(" \n[\"\\u0074\",1,0]\t".into())).unwrap(),
            Some(PriorityValue::Timestamp(UtcTimestamp::new(1, 0).unwrap()))
        );
        // The public/log human-readable format remains the original enum object.
        assert_eq!(
            to_json(&PriorityValue::Timestamp(UtcTimestamp::new(1, 0).unwrap())).unwrap(),
            r#"{"Timestamp":{"seconds":1,"nanoseconds":0}}"#
        );
    }

    #[test]
    fn malformed_compact_priorities_fail_without_truncation_or_coercion() {
        for raw in [
            "[]",
            r#"["x",1]"#,
            r#"[null,1]"#,
            ...
            r#"["i",true]"#,
            r#"["i","1"]"#,
            r#"["i",9223372036854775808]"#,
            r#"["i",1,2]"#,
            r#"["d",1]"#,
            r#"["d",1,-1]"#,
            r#"["d",170141183460469231731687303715884105728,0]"#,
            r#"["s",1]"#,
            r#"["s","x",null]"#,
            r#"["s","x"] true"#,
        ] {
            assert!(parse_priority(Some(raw.into())).is_err(), "accepted {raw}");
        }
    }
}
