            ...
            futures::executor::block_on(async {
                ...
                backend
                    .push_with_request_id(
                        ...
                        vec![
                            fireweed_engine::PushSpec::default(),
                            fireweed_engine::PushSpec::default(),
                        ],
                        fireweed_conformance::ts(0),
                        None,
                    ...
                    .unwrap();
                backend
                    .claim(fireweed_conformance::claim_req(1, 500, 10))
                    .await
                    .unwrap();
                assert_eq!(
                    backend
                        .current_position(&fireweed_conformance::shard())
                        .await
                        .unwrap(),
                    // CreateQueue is the atomic seq-0 command; push is seq 1 and claim is seq 2.
                    CommandPosition::new(fireweed_conformance::shard(), 0, 2)
                );
            });
...
fn postgres_relational_truncate_then_recover_exact_state() {
    match std::env::var("PQUEUE_PG_TEST_URL") {
        Ok(url) => {
            ...
            let body = vec![
                fireweed_engine::PushSpec {
                    priority: Some(fireweed_core::PriorityValue::Int64(10)),
                    ..Default::default()
                },
                fireweed_engine::PushSpec {
                    priority: Some(fireweed_core::PriorityValue::Int64(20)),
                    ..Default::default()
                },
            ];
            let (original_ids, fence_epoch) = futures::executor::block_on(async {
                ...
                let fence_epoch = backend
                    .acquire_epoch(&fireweed_conformance::shard())
                    .await
                    .unwrap();

                let metrics = backend
                    .metrics(&fireweed_conformance::shard())
                    .await
                    .unwrap();
                assert_eq!((metrics.pending, metrics.leased), (1, 1));
                assert_eq!(
                    backend
                        .current_epoch(&fireweed_conformance::shard())
                        .await
                        .unwrap(),
                    fence_epoch
                );
            ...
            futures::executor::block_on(async {
                ...
                assert!(
                    backend
                        .pending(&fireweed_conformance::shard())
                        .await
                        .unwrap()
                        .is_empty(),
                ...
                let fresh = backend
                    .push(
                        &fireweed_conformance::shard(),
                        vec![fireweed_engine::PushSpec::default()],
                        fireweed_conformance::ts(2),
                        None,
                    )
                    .await
...
use fireweed_core::{ClientItemKey, IndexDeclaration, IndexType, QueueIndex};
use fireweed_engine::{IndexQueryPort, PayloadUpdate, PurgePort, UpdateFieldsPort, UpsertPort};
use std::collections::BTreeMap;

fn pg_qdef_unique_str_index(index_name: &str, field: &str) -> fireweed_core::QueueDefinition {
    fireweed_core::QueueDefinition {
        ...
        ..fireweed_conformance::qdef()
    }
}

fn pg_qdef_nonunique_str_index(index_name: &str, field: &str) -> fireweed_core::QueueDefinition {
    fireweed_core::QueueDefinition {
        typed_indexes: vec![QueueIndex {
            name: index_name.to_string(),
            declaration: IndexDeclaration::Single(IndexDef {
    ...
    match std::env::var("PQUEUE_PG_TEST_URL") {
        Ok(url) => {
            ...
            futures::executor::block_on(async {
                let b = pg_typed_connect(&url, &schema);
                let shard = fireweed_conformance::shard();
                b.create_queue(fireweed_conformance::qdef()).await.unwrap();
                let ids = b
                    .push(
                        &shard,
                        vec![fireweed_engine::PushSpec::default()],
