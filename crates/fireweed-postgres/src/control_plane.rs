...
//! reimplemented here — they live once in `fireweed-engine`'s pure lease decisions
...
impl QueueControlPlane for PostgresControlPlane {
    ...
    fn renew_queue_leases(
        ...
        let expires_nanos = ts_nanos(fireweed_engine::add_millis(now, self.config.lease_ttl_ms));
