...
//! priority-ordered **projection is the shared [`fireweed_projection::ProjectionData`] materialization,
//! rebuilt from the log**. The log rows are the source of truth (CQRS); the in-memory projection is a
//! derived view that any reopen reconstructs via [`fireweed_projection::ProjectionData::apply_command`].
//! This is the [`fireweed_sqlite`]-established durable-adapter template over the SYNC `postgres` client —
...
//! PRODUCTION `fireweed-server` (whose RESP loop + reclaim driver run under tokio), this backend MUST be
//! driven off the runtime. That is now done: `fireweed-server`'s `PostgresNativeBackend` wrapper (behind its
...
/// credential-provider-aware path the `fireweed-server` composition root uses for Lakebase), with
/// recovery-on-open.
...
impl PostgresBackend {
    ...
    /// `fireweed-server` composition root uses for Lakebase (a libpq/native-password DSN plus, optionally, a
    ...
    ) -> impl std::future::Future<Output = EngineResult<fireweed_engine::RawCommitOutcome>> + Send
    {
        let result = (|| {
            ...
            if fault == fireweed_engine::RawCommitFault::AfterAppendBeforeApply {
                return Ok(fireweed_engine::RawCommitOutcome::appended(positions));
            }
...
/// [`fireweed_engine::EngineError::Unavailable`] so a caller rejects it before activation.
impl fireweed_engine::CommitTransitionPort for PostgresBackend {}
...
/// log-replay backends.
impl fireweed_engine::HotProjectionQueryPort for PostgresBackend {
    fn metrics_by_query(
        &self,
        shard: &QueueKey,
