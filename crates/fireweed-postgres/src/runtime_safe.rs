...
/// Forward snapshot lifecycle to the inner product backend off the reactor.
impl<B: ProductBackend + SnapshotStore + 'static> SnapshotStore for RuntimeSafeBackend<B> {
    fn write_snapshot(
        &self,
        shard: &QueueKey,
        position: CommandPosition,
    ...
    fn read_snapshot(
        ...
        let q = r.queue.clone();
        self.offload(q, move |i| async move {
