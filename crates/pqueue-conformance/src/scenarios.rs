...
use crate::{claim_req, commit, envelope, item, qdef, qkey, shard, ts, ConformanceBackend};
...
/// default/empty no-op).
pub async fn peek_is_priority_ordered_and_nondestructive<B: ConformanceBackend>(make: impl Fn() -> B) {
    let b = make();
    b.create_queue(qdef()).await.unwrap();
    commit(
        &b,
    ...
    assert_eq!(peeked, vec!["b", "c", "a"], "peek is priority-ordered (10,20,30)");
    // Non-destructive: peeking again returns the same items (peek must not consume/claim).
    assert_eq!(b.peek(&shard(), 10).await.unwrap().len(), 3, "peek does not consume");
    assert_eq!(b.peek(&shard(), 1).await.unwrap().len(), 1, "peek honors the limit");
}
...
pub async fn pending_lists_leased_items<B: ConformanceBackend>(make: impl Fn() -> B) {
    ...
    .await;
    assert!(b.pending(&shard()).await.unwrap().is_empty(), "nothing leased yet");
...
pub async fn snapshots_write_read_latest<B: ConformanceBackend>(make: impl Fn() -> B) {
    ...
    b.create_queue(qdef()).await.unwrap();
    let sk = shard();
    assert!(b.latest_snapshot(&sk).await.unwrap().is_none(), "no snapshot yet");
    ...
    assert_eq!(
        b.latest_snapshot(&sk).await.unwrap().expect("a snapshot exists").ref_id,
        r2.ref_id,
...
pub async fn claim_then_complete_lifecycle<B: ConformanceBackend>(make: impl Fn() -> B) {
    ...
    let m = b.metrics(&qkey()).await.unwrap();
    assert_eq!(m.complete, 1, "finalize-complete must move item to complete");
...
pub async fn claim_returns_priority_ordered_rich_items<B: ConformanceBackend>(make: impl Fn() -> B) {
    let b = make();
    b.create_queue(qdef()).await.unwrap();
    commit(
        &b,
    ...
    assert_eq!(ids, vec!["b", "c"], "claim must deliver highest priority first");
...
pub async fn paused_queue_yields_no_claims<B: ConformanceBackend>(make: impl Fn() -> B) {
    ...
    assert!(b.claim(claim_req(10, 500, 10)).await.unwrap().items.is_empty());
    assert!(
    ...
    assert_eq!(b.claim(claim_req(10, 500, 10)).await.unwrap().items.len(), 1);
}
...
pub async fn pause_and_fence_reconstruct_from_log<B: ConformanceBackend>(make: impl Fn() -> B) {
    ...
    assert!(
        b.claim(claim_req(10, 500, 50)).await.unwrap().items.is_empty(),
        "pause reconstructed"
    );
    let outcomes = vec![FinalizeOutcome {
        item_id: aid,
