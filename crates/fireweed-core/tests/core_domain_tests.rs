...
fn core_domain_tests_rejects_mutually_exclusive_recurrence_and_cohort_fields() {
    ...
    request.recurrence = RecurrencePolicy {
        ...
        until: Some(fireweed_core::UtcTimestamp::new(1_700_000_000, 0).unwrap()),
    };

    let error = request.validate(&policy()).unwrap_err();
    assert_eq!(
        error.kind,
        fireweed_core::CreateQueueErrorKind::InvalidRequest
    );
    assert!(
...
fn core_domain_tests_rejects_completion_bound_above_progress_bound() {
    ...
    assert_eq!(
        error.kind,
        fireweed_core::CreateQueueErrorKind::QueueDefinitionConflict
    );
    assert!(
        error
            .message
...
fn core_domain_tests_applies_policy_defaults() {
    let mut request = valid_create_queue();
    request.eligibility_policy = fireweed_core::EligibilityPolicy {
...
fn core_domain_tests_queue_definition_serde_defaults_entity_schema_and_typed_indexes() {
    ...
    let restored: fireweed_core::QueueDefinition = serde_json::from_value(json).unwrap();
...
fn core_domain_tests_conflict_typed_and_secondary_same_name() {
    let mut request = valid_create_queue();
    request.secondary_indexes = vec![fireweed_core::IndexSpec {
