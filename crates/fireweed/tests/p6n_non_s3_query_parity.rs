...
async fn run_cell(cell: &str, expect_projection_control: bool, open_fw: impl FnOnce() -> Fireweed) {
    let fw = open_fw();
    public_interface::run_p6_surface(cell, &fw, expect_projection_control).await;
    drop(fw);
}
...
#[tokio::test]
async fn p6n_memory_memory_query_parity() {
...
#[tokio::test]
async fn p6n_filesystem_memory_query_parity() {
...
mod postgres_cells {
    ...
    async fn p6n_memory_postgres_query_parity() {
        let url = pg_url();
        let mut cfg = StorageConfig::memory();
        cfg.projection = ProjectionStoreConfig::Postgres {
            url: ConfigSecret::new(url),
        };
        cfg.namespace = format!("p6n_mem_pg_{}", std::process::id());
    ...
    async fn p6n_postgres_memory_query_parity() {
        ...
        let schema = format!("p6n_pg_mem_{}", std::process::id());
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn p6n_postgres_postgres_query_parity() {
        let url = pg_url();
        // Isolate relational schema so concurrent matrix runs do not collide.
        let schema = format!("p6n_pg_pg_{}", std::process::id());
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn p6n_filesystem_postgres_query_parity() {
        let url = pg_url();
        ...
        let cfg = fireweed::ObjectLogRuntimeConfig {
            ...
            response_barrier: ResponseBarrier::Strict,
            segments: segments(),
            namespace: format!("p6n-fs-pg-{}", std::process::id()),
            recovery: fireweed::RecoveryPolicy::default(),
        };
