...
fn require_s3_env() -> (String, String, String, String, String) {
    let endpoint = std::env::var("FIREWEED_S3_TEST_ENDPOINT")
        .expect("FIREWEED_S3_TEST_ENDPOINT required for P8S3 (P1s provenance; zero skips)");
    let bucket = std::env::var("FIREWEED_S3_TEST_BUCKET")
        .expect("FIREWEED_S3_TEST_BUCKET required for P8S3 (P1s provenance; zero skips)");
    ...
    let access = std::env::var("FIREWEED_S3_TEST_ACCESS_KEY")
        .expect("FIREWEED_S3_TEST_ACCESS_KEY required for P8S3 (P1s provenance; zero skips)");
    let secret = std::env::var("FIREWEED_S3_TEST_SECRET_KEY")
        .expect("FIREWEED_S3_TEST_SECRET_KEY required for P8S3 (P1s provenance; zero skips)");
    (endpoint, bucket, region, access, secret)
}

fn require_pg_url() -> String {
    std::env::var("FIREWEED_PG_TEST_URL")
        .expect("FIREWEED_PG_TEST_URL required for P8S3 s3×postgres (zero skips)")
}

fn load_attestation() -> Value {
    let text = std::fs::read_to_string(ATTESTATION_PATH).unwrap_or_else(|error| {
        panic!("P8S3 requires P1s attestation at {ATTESTATION_PATH}: {error}")
    });
...
fn require_p1s_native_cas_provenance() {
    ...
    assert!(
        ...
        "P8S3 requires attested native CAS create+update; create={native_create} update={native_update}"
    );
    ...
    eprintln!("P8S3 provenance: endpoint={endpoint} bucket={bucket} native_cas=create+update");
}
...
fn unique_ns(label: &str) -> String {
    ...
    let nanos = SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    format!("p8s3-{label}-{}-{n}-{nanos}", std::process::id())
}
...
async fn run_p8(cell_id: &str, config: StorageConfig) {
    let fireweed = open_cell(cell_id, config).await;
    public_interface::run_p8_surface(cell_id, &fireweed).await;
    eprintln!("P8S3 PASS {cell_id} run_p8_surface");
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn s3_memory_strict_p8_mutation_parity() {
    require_p1s_native_cas_provenance();
    let ns = unique_ns("s3-memory");
    let config = s3_log_config(ns, ProjectionStoreConfig::Memory);
    run_p8("s3--memory--strict", config).await;
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn s3_postgres_strict_p8_mutation_parity() {
    require_p1s_native_cas_provenance();
    ...
    run_p8("s3--postgres--strict", config).await;
}
