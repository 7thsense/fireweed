//! P9S3 — S3 queue, ownership, read, discovery, and transaction parity.
//!
//! Executable boundary (fireweed-f3cb9ad9): on the three S3 cells
//! (`s3×memory`, `s3×sqlite`, `s3×postgres`) run exactly P6N's applicable
//! assertion/method set (`public_interface::run_p9_surface`) using P5aS3 open
...
//!   --test p9s3_s3_query_parity -- --nocapture
//! ```
...
fn require_s3_env() -> (String, String, String, String, String) {
    let endpoint = std::env::var("FIREWEED_S3_TEST_ENDPOINT")
        .expect("FIREWEED_S3_TEST_ENDPOINT required for P9S3 (P1s provenance; zero skips)");
    let bucket = std::env::var("FIREWEED_S3_TEST_BUCKET")
        .expect("FIREWEED_S3_TEST_BUCKET required for P9S3 (P1s provenance; zero skips)");
    ...
    let access = std::env::var("FIREWEED_S3_TEST_ACCESS_KEY")
        .expect("FIREWEED_S3_TEST_ACCESS_KEY required for P9S3 (P1s provenance; zero skips)");
    let secret = std::env::var("FIREWEED_S3_TEST_SECRET_KEY")
        .expect("FIREWEED_S3_TEST_SECRET_KEY required for P9S3 (P1s provenance; zero skips)");
...
fn load_attestation() -> Value {
    let text = std::fs::read_to_string(ATTESTATION_PATH).unwrap_or_else(|error| {
        panic!("P9S3 requires P1s attestation at {ATTESTATION_PATH}: {error}")
    });
...
fn require_p1s_native_cas_provenance() {
    ...
    assert!(
        ...
        "P9S3 requires attested native CAS create+update; create={native_create} update={native_update}"
    );
    ...
    eprintln!("P9S3 provenance: endpoint={endpoint} bucket={bucket} native_cas=create+update");
}
...
fn unique_ns(label: &str) -> String {
    ...
    let nanos = SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    format!("p9s3-{label}-{}-{n}-{nanos}", std::process::id())
}
...
async fn run_p9(cell_id: &str, config: StorageConfig) {
    let fireweed = open_cell(cell_id, config).await;
    public_interface::run_p9_surface(cell_id, &fireweed, true).await;
    eprintln!("P9S3 PASS {cell_id} run_p9_surface");
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn s3_memory_strict_p9_transaction_parity() {
    require_p1s_native_cas_provenance();
    let ns = unique_ns("s3-memory");
    let config = s3_log_config(ns, ProjectionStoreConfig::Memory);
    run_p9("s3--memory--strict", config).await;
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn s3_sqlite_strict_p9_transaction_parity() {
    require_p1s_native_cas_provenance();
    ...
    run_p9("s3--sqlite--strict", config).await;
}
...
async fn s3_postgres_strict_p9_transaction_parity() {
    ...
    run_p9("s3--postgres--strict", config).await;
}
