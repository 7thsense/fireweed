...
fn main() {
    ...
    let attestation = EvidenceAttestation {
        ...
        producing_command:
            "scripts/release/build-governed-evidence-bundle.sh + pqueue-build-evidence-attestation"
                .into(),
        produced_at,
        reviewed_at,
        evidence,
...
fn usage() -> ! {
    eprintln!(
        "usage: pqueue-build-evidence-attestation --repo-root <dir> --bundle <repo-relative-dir> --tag <tag> --commit <sha> --produced-at <UTC> --reviewed-at <UTC> --out <path>"
    );
    std::process::exit(2)
}
