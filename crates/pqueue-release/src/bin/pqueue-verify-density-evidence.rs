...
fn main() {
    let path = std::env::args_os()
        ...
        .map(PathBuf::from)
        .unwrap_or_else(|| {
            eprintln!("usage: pqueue-verify-density-evidence <ledger.jsonl>");
            std::process::exit(2);
        });
    ...
    for (index, line) in contents.lines().enumerate() {
        ...
        rows += 1;
        match serde_json::from_str::<pqueue_release::LedgerRow>(line) {
            Ok(row) => {
                if let Err(row_errors) = pqueue_release::density::validate_release_row(&row) {
                    errors.extend(
                        row_errors
                            .into_iter()
