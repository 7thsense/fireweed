...
use fireweed_engine::{ClaimPort, ControlPlaneStore, ProjectionRead, PushCommand, QueueCommand};
use fireweed_sqlite::{composed_sqlite_backend, composed_sqlite_backend_in_memory};

fn temp_db(tag: &str) -> std::path::PathBuf {
    std::env::temp_dir().join(format!("fireweed-sqlite-{tag}-{}.db", std::process::id()))
}
...
async fn snapshots_round_trip_and_latest_is_most_recent() {
    use fireweed_engine::{ProjectionSnapshot, SnapshotStore};
    let b = composed_sqlite_backend_in_memory().expect("open");
    b.create_queue(qdef()).await.unwrap();
    let sk = fireweed_conformance::shard();
    let pos = fireweed_engine::CommandPosition::new(sk.clone(), 0, 0);
    let r1 = b
        .write_snapshot(
            &sk,
            pos.clone(),
...
async fn high_water_persists_across_reopen() {
    use fireweed_engine::SnapshotStore;
    ...
    let before = {
        ...
        .await;
        b.high_water(&fireweed_conformance::shard())
            .await
            .unwrap()
...

    {
        let b = composed_sqlite_backend(p).expect("reopen");
        let after = b
            .high_water(&fireweed_conformance::shard())
            .await
            .unwrap()
