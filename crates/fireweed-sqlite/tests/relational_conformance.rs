...
fireweed_conformance::adr011_typed_conformance_suite!(make);
fireweed_conformance::claimed_item_shape_conformance_tests!(@whole_cohort make);
...
mod composed {
    ...
    fireweed_conformance::core_suite!(@atomic || composed_sqlite_relational_in_memory()
...
mod composed_log_projection {
    use fireweed_sqlite::composed_sqlite_log_sqlite_projection_in_memory;
    fireweed_conformance::core_suite!(@atomic || composed_sqlite_log_sqlite_projection_in_memory()
        .expect("compose in-memory sqlite-log + sqlite-projection backend"));
    fireweed_conformance::log_replay_suite!(|| composed_sqlite_log_sqlite_projection_in_memory()
        .expect("compose in-memory sqlite-log + sqlite-projection backend"));
}

#[tokio::test]
async fn sqlite_relational_log_replay_branch_read_as_of() {
    use fireweed_engine::{HistoricalProjectionRead, ProjectionRead, ProjectionStore, PushPort};

    let backend = fireweed_sqlite::composed_sqlite_log_sqlite_projection_in_memory()
        .expect("compose in-memory sqlite-log + sqlite-projection backend");
    backend.create_queue(qdef()).await.unwrap();
    ...
    let branched: Vec<fireweed_engine::ItemView> = backend
...
async fn sqlite_relational_conformance_historical_projection_read_is_explicit() {
    ...
    let err = b
        .read_as_of(
            &shard,
            position,
            |_projection| -> fireweed_engine::EngineResult<Vec<fireweed_engine::ItemView>> {
    ...
    assert!(
        matches!(
            b.renew(&shard(), vec![item_id], ts(600), ts(110), None)
                .await,
            Err(fireweed_engine::EngineError::Invalid(_))
        ),
...
use fireweed_engine::IndexQueryPort;
...
async fn seeded_commit_transition_sqlite_relational_backend() -> SqliteRelationalBackend {
    use bytes::Bytes;
    ...
    use fireweed_engine::{
        ...
        FinalizeKind, InstanceFence, PushPort, PushSpec, SideRecord,
    };
...
/// Shared commit-transition positive scenario against the durable sqlite-relational composition root.
#[tokio::test]
async fn commit_transition_shared_scenario_runs_against_sqlite_relational() {
    ...
    let make = move || match make_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst) {
        0 => SqliteRelationalBackend::in_memory().unwrap(),
        _ => std::thread::spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .unwrap()
                .block_on(seeded_commit_transition_sqlite_relational_backend())
        })
        .join()
        .unwrap(),
    };
