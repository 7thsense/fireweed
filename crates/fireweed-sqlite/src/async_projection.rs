...
use fireweed_core::{ItemId, ItemState, QueueDefinition, RequestId, UtcTimestamp};
use fireweed_engine::{
...
use fireweed_projection::ProjectionImage;
...
const WORKER_NAME: &str = "fireweed-sqlite-projection";
...
impl AsyncProjectionStore for AsyncSqliteProjectionStore {
    ...
    ) -> impl Future<Output = EngineResult<Vec<fireweed_engine::FinalizeLeaseMember>>> + Send {
        let actor = self.clone();
        async move {
            actor
    ...
    fn cohort_lease_validate(
        ...
        now: UtcTimestamp,
    ) -> impl Future<Output = EngineResult<Vec<fireweed_engine::CohortLeaseMember>>> + Send {
        let actor = self.clone();
        async move {
            actor
...
mod tests {
    ...
    async fn purge_validation_and_apply_handle_missing_dedup_and_second_noop() {
        ...
        let env = CommandEnvelope {
            ...
            request_outcome: None,
            item_ids: vec![item],
            command: QueueCommand::PurgeItems(fireweed_engine::PurgeItemsCommand {
                item_ids: vec![item],
                force: false,
            }),
            checksum: CommandChecksum(0),
    ...
    async fn active_lease_reopen_uses_durable_hash_for_renew_validation() {
        ...
        assert_eq!(
            reopened
                ...
                .await
                .unwrap(),
            vec![fireweed_engine::FinalizeLeaseMember {
                item_id: item,
                attempt_count: 1,
                max_attempts: 3,
            }]
    ...
    async fn push_capabilities_are_read_only_and_follow_durable_projection_state() {
        ...
        let recorded_fingerprint = match &recorded.command {
            QueueCommand::Push(command) => PushFingerprint {
                canonical_sha256: fireweed_engine::push_items_fingerprint_sha256(&command.items)
                    .unwrap(),
