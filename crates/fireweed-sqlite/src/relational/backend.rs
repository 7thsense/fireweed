...
fn index_field_type<'a>(spec: &'a fireweed_core::QueueIndex, field: &str) -> Option<&'a IndexType> {
    index_fields(spec)
        .into_iter()
...
) -> EngineResult<fireweed_core::SortDirection> {
...
fn resolving_query_index<'a>(
    ...
    preferred: &'a fireweed_core::QueueIndex,
    filters: &[fireweed_core::QueryFilter],
) -> EngineResult<&'a fireweed_core::QueueIndex> {
...
fn hot_query_candidate_page(
    ...
    spec: &fireweed_core::QueueIndex,
    filters: &[fireweed_core::QueryFilter],
    direction: fireweed_core::SortDirection,
...
fn hot_query_candidates(
    ...
    loop {
        let page = hot_query_candidate_page(
            ...
            spec,
            filters,
            fireweed_core::SortDirection::Ascending,
            cursor
                .as_ref()
...
/// Relational SQLite does not yet own a disposable projection snapshot plane; snapshot APIs
/// fail closed so sole-owner log-replay backends remain the supported snapshot path.
impl fireweed_engine::SnapshotStore for SqliteRelationalBackend {
    fn write_snapshot(
        &self,
        _shard: &QueueKey,
...
impl fireweed_engine::CommitTransitionPort for SqliteRelationalBackend {
...
impl fireweed_engine::HotProjectionQueryPort for SqliteRelationalBackend {
...
impl fireweed_engine::HistoricalProjectionRead for SqliteRelationalBackend {
...
mod hot_query_sql_tests {
    ...
    async fn grouped_claim_finalize_statement_count(groups: usize) -> (usize, usize) {
        use fireweed_engine::{FinalizeKind, FinalizeOutcome, FinalizePort};
    ...
    async fn range_cursor_rejects_a_deleted_anchor() {
        ...
        let request = RangeScanRequest {
            ...
            order_by: vec![fireweed_core::OrderField {
                field: "status".into(),
                direction: fireweed_core::SortDirection::Ascending,
            }],
            page_size: 1,
    ...
    fn invalid_index_shapes_are_rejected_before_sql() {
        let spec = QueueIndex {
            name: "compound".into(),
            declaration: IndexDeclaration::Compound(fireweed_core::CompoundIndexDef {
                fields: vec![
                    fireweed_core::CompoundIndexField {
                        field: "a".into(),
                        index_type: IndexType::String,
                    },
                    fireweed_core::CompoundIndexField {
