...
const WORKER_NAME: &str = "pqueue-postgres-projection";
...
impl AsyncPostgresRelationalProjection {
    ...
    pub async fn connect_with_capacity(url: &str, mailbox_capacity: usize) -> EngineResult<Self> {
        ...
        let (actor, opened) =
            ActorHandle::spawn(mailbox_capacity, move || PostgresRelational::connect(&url))?;
        opened.await?;
        Ok(Self { actor })
    }
...
mod tests {
    ...
    fn rejects_zero_capacity_without_spawning() {
        let result = ActorHandle::<()>::spawn(0, || {
    ...
    fn opening_failure_is_reported_and_drain_resolves() {
        let (actor, opened) =
            ActorHandle::<()>::spawn(1, || Err(EngineError::Storage("open failed".to_string())))
                .unwrap();
        assert!(matches!(
