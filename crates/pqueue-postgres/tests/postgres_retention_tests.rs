...
async fn postgres_retention_tests_request_id_expired_record_deleted() {
    ...
    let client = c.lock().await;
    let deleted = retention::expire_request_idempotency(&client, now())
        .await
        .unwrap();
...
async fn postgres_retention_tests_request_id_active_record_preserved() {
    ...
    assert_eq!(
        count, 1,
        "active request_id record must survive GC (INV-5: replay window open)"
    );
}
...
async fn postgres_retention_tests_item_key_expired_record_deleted() {
    ...
    cs.create_queue(simple_def(tid("t"), qid("q-ikr-exp")))
        .await
        .unwrap();
    ap.batch_push(push_req("t", "q-ikr-exp", "item-1", "key-ikr-1"))
        .await
        .unwrap();
    ...
    let client = c.lock().await;
    let deleted = retention::expire_item_key_retention(&client, now())
        .await
        .unwrap();
...
async fn postgres_retention_tests_item_key_active_record_preserved() {
    ...
    cs.create_queue(simple_def(tid("t"), qid("q-ikr-act")))
        .await
        .unwrap();
    ap.batch_push(push_req("t", "q-ikr-act", "item-2", "key-ikr-2"))
        .await
        .unwrap();
...
async fn postgres_retention_tests_terminal_expired_item_deleted() {
    ...
    cs.create_queue(simple_def(tid("t"), qid("q-term-exp")))
        .await
        .unwrap();
    ap.batch_push(push_req("t", "q-term-exp", "item-t1", "key-t1"))
        .await
        .unwrap();
...
async fn postgres_retention_tests_terminal_recent_item_preserved() {
    ...
    cs.create_queue(simple_def(tid("t"), qid("q-term-act")))
        .await
        .unwrap();
    ap.batch_push(push_req("t", "q-term-act", "item-t2", "key-t2"))
        .await
        .unwrap();
    ...
    assert_eq!(
        deleted, 0,
        "recently terminal item must not be deleted (still within window)"
    );
...
async fn postgres_retention_tests_terminal_pending_items_untouched() {
    ...
    cs.create_queue(simple_def(tid("t"), qid("q-pend")))
        .await
        .unwrap();
    ap.batch_push(push_req("t", "q-pend", "item-p1", "key-p1"))
        .await
        .unwrap();

    // Try to GC with a very permissive cutoff — item is pending, not terminal.
    let client = c.lock().await;
    let deleted = retention::expire_terminal_items(&client, "t", "q-pend", future(9999))
        .await
        .unwrap();
    assert_eq!(
        deleted, 0,
        "pending items must never be deleted by terminal retention GC"
    );
}
...
async fn postgres_retention_tests_inv5_terminal_gc_does_not_delete_command_log() {
    ...
    cs.create_queue(simple_def(tid("t"), qid("q-inv5")))
        .await
        .unwrap();
    let push_result = ap
        .batch_push(push_req("t", "q-inv5", "item-inv5", "key-inv5"))
        .await
        .unwrap();
