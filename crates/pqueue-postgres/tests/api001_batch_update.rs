...
//!
//! Run with:
//! `PQUEUE_PG_TEST_URL=postgres://postgres:pq@127.0.0.1:5433/postgres cargo test -p pqueue-postgres --test api001_batch_update`
...
fn qdef(max_batch: usize) -> pqueue_core::QueueDefinition {
...
fn claim_request(shard: &QueueKey, token: &str, now: i64) -> ClaimRequest {
    ClaimRequest {
        ...
        max_items: 1,
        lease_token: pqueue_core::LeaseToken::new(token).unwrap(),
        lease_expires_at: pqueue_conformance::ts(now + 100),
        now: pqueue_conformance::ts(now),
...
fn batch_update_is_set_based_at_sizes_1_100_and_1000() {
    ...
    for size in [1_usize, 100, 1_000] {
        ...
        futures::executor::block_on(async {
            ...
            let ids = backend
                .push(&shard, pushes(size), pqueue_conformance::ts(0), None)
                .await
                .unwrap();
...
fn batch_update_preserves_order_and_idempotency_across_mixed_outcomes() {
    ...
    futures::executor::block_on(async {
        ...
        first.not_before = BatchUpdateValue::Replace(Some(pqueue_conformance::ts(50)));
        ...
        let response = backend
            .batch_update(&shard, request.clone(), pqueue_conformance::ts(4), None)
            .await
            .unwrap();
        assert_eq!(
            response.results,
        ...
        let replay = backend
            .batch_update(&shard, request.clone(), pqueue_conformance::ts(5), None)
            .await
            .unwrap();
        ...
        assert_eq!(
            backend
                .batch_update(&shard, changed, pqueue_conformance::ts(5), None)
                .await,
...
fn stale_epoch_and_snapshot_tail_rebuild_preserve_batch_update_replay() {
    ...
    let (successful_response, rejected_response) = futures::executor::block_on(async {
        ...
        backend.create_queue(qdef(100)).await.unwrap();
        let ids = backend
            .push(&shard, pushes(1), pqueue_conformance::ts(0), None)
            .await
            .unwrap();
    ...
    futures::executor::block_on(async {
        ...
        assert_eq!(
            reopened
                .batch_update(&shard, successful, pqueue_conformance::ts(4), None)
                .await
                .unwrap(),
            successful_response
        );
        assert_eq!(
            reopened
                .batch_update(&shard, rejected, pqueue_conformance::ts(4), None)
                .await
                .unwrap(),
            rejected_response
        );
