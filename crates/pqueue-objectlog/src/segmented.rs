...
impl<S: BlobStore> SegmentedObjectLog<S> {
    ...
    pub fn expire_segments_through(
        ...
        // so the collision fence stays intact. The current index-CAS manifest protocol therefore exposes the
        // eligible candidates here, but it does not free those manifest addresses in this bead.
    ...
    fn advance_read_horizon_from_entries(
        ...
        let (_, new_w) = self.manifest_reclamation_candidates_from_entries(
            shard,
            reclaimed_through,
            now_ms,
            entries,
        ...
        {
            // Best-effort monotonic PUT. A candidate is always derived strictly below the durable floor and
            // from a candidate set that only includes reclaimed, unpinned entries (see SAFETY above), so even
            // a non-atomic read-check-then-put that a racing writer interleaves can only regress W to another
            // below-floor value — never above a live entry.
