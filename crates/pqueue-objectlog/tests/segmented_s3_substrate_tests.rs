...
/// Test 10 — when below-floor segment reclamation fails partway through, the durable watermark does not skip
/// the remaining undeleted below-floor segments and a retry resumes from the last committed watermark.
#[test]
#[allow(non_snake_case)]
...
fn partial_expire_does_not_hide_undeleted_below_floor_segments() {
    TestManifestDeletionWatermarkPartialExpiryRetry();
}
