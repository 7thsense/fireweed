...
use pqueue_objectlog::segmented::{InMemoryBlobStore, SegmentConfig, SegmentedObjectLog};
...
fn push_envelopes(n: u64) -> Vec<CommandEnvelope> {
    (0..n)
        .map(|_| CommandEnvelope {
            ...
            checksum: pqueue_engine::CommandChecksum(0),
            created_at: pqueue_core::UtcTimestamp::new(0, 0).unwrap(),
        })
        .collect()
}
