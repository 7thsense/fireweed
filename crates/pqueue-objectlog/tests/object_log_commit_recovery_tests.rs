...
fn TestPartialExpireRecoveryKeepsVisibleUndeletedSegments() {
    ...
    assert_eq!(
        ...
        None,
        "a delete failure before any reclaimed segment commits no watermark"
    );
    ...
    assert_eq!(
        reopened.read_read_horizon(&shard).unwrap(),
        None,
        "reopen preserves the absence of a watermark when no segment delete completed"
    );
    ...
    assert_eq!(
        live_tail
            ...
            .collect::<Vec<_>>(),
        vec![4, 5, 6, 7],
        "the undeleted live tail remains readable after the interrupted expire"
    );
    assert!(
