...
use pqueue_core::{ItemId, LeaseToken, QueryCapabilityFlags, RangeScanRequest, RequestId};
use pqueue_engine::{
    ...
    B: Backend + CommitTransitionPort + RecoveryReadPort,
{
    ...
    let err = backend
        .side_record(&pqueue_conformance::qkey(), b"state/run")
        .await
        .unwrap_err();
    assert_eq!(err, EngineError::Unavailable);
}
