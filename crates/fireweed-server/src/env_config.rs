...
/// set fireweed-server actually wires. The `PQUEUE_OBJECT_LOG_MODE` pseudo-axis is retired — the object log's
...
fn parse_backend(
    ...
    if !wired {
        return Err(unsupported_storage(
            &log,
            &projection,
            "this PQUEUE_LOG_BACKEND × PQUEUE_PROJECTION_BACKEND pairing is not wired by fireweed-server",
        ));
    }
