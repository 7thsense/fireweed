...
use fireweed_objectlog::{ComposedObjectLogBackend, ObjectLog, segmented::SegmentCounters};
use fireweed_server::{SegmentConfig, SegmentedObjectLogSqliteBackend};
use fireweed_sqlite::{
...
impl ObjectStoreUtilization {
    ...
    fn cost_per_million_resident_items_usd(
        self,
        resident: u64,
        prices: &fireweed_release::cost::PriceInputs,
    ) -> f64 {
        if resident == 0 {
            return 0.0;
        }
...
fn s3_cost_breakdown(
    ...
    let storage_gb = m.total_bytes as f64 / fireweed_release::cost::BYTES_PER_GB;
    let month_fraction = m.retained_hours / fireweed_release::cost::HOURS_PER_MONTH;
...
fn apply_object_store_counters(
    ...
    let object_store = ObjectStoreUtilization {
        ...
        retained_hours: fireweed_release::cost::HOURS_PER_MONTH,
    };
    let prices = fireweed_release::cost::PriceInputs::adr_001_us_east_1();
...
async fn run_inmemory(
    ...
    let backend = Arc::new(
        fireweed_objectlog::composed_objectlog_backend_group_commit(&root, cfg)
...
fn emit_ledger(
    ...
    fireweed_release::append_row(&path, &row).expect("append hybrid ledger row");
    fireweed_release::verify_ledger(&path, true).expect("strict hybrid ledger validates");
...
fn emit_matrix_cell(
    ...
    let path = fireweed_release::ledger_path(env!("CARGO_MANIFEST_DIR"), suite);
    fireweed_release::append_row(&path, &row).expect("append matrix ledger cell");
}
...
fn emit_scale_cell(
    ...
    let path = fireweed_release::ledger_path(env!("CARGO_MANIFEST_DIR"), suite);
    fireweed_release::append_row(&path, &row).expect("append scale ledger cell");
}
...
fn object_store_utilization_s3_cost_uses_request_and_storage_inputs() {
    ...
    let metrics = ObjectStoreUtilization {
        ...
        list_count: 3_000,
        delete_count: 4_000,
        retained_hours: fireweed_release::cost::HOURS_PER_MONTH / 2.0,
    };
...
async fn performance_object_log_hybrid_attribution() {
    ...
    let command = "cargo test -p fireweed-server --release --test performance_object_log_hybrid_tests performance_object_log_hybrid_attribution -- --nocapture";
...
async fn performance_object_log_hybrid_bounded_debt_gate() {
    ...
    let command = "cargo test -p fireweed-server --release --test performance_object_log_hybrid_tests performance_object_log_hybrid_bounded_debt_gate -- --nocapture";
    let gates = run_suite_named(suite, command, false).await;
...
async fn performance_object_log_hybrid_segment_density_gate() {
    ...
    let command = "cargo test -p fireweed-server --release --test performance_object_log_hybrid_tests performance_object_log_hybrid_segment_density_gate -- --nocapture";
    let gates = run_suite_named(suite, command, false).await;
...
async fn performance_object_log_hybrid_cache_matrix_smoke() {
    ...
    fireweed_release::verify_ledger(&path, true).expect("strict cache-matrix ledger validates");
...
async fn performance_object_log_hybrid_scale_matrix() {
    ...
    if ran > 0 {
        fireweed_release::verify_ledger(&path, true).expect("strict scale-matrix ledger validates");
...
/// `ObjectLog::read_from` (crates/fireweed-objectlog/src/compose_log.rs) used to advance its resume cursor one
