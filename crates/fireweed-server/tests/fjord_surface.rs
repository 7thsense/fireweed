...
fn change_record(
    ...
    ChangeRecord {
        ...
        item_version: Some(1),
        terminal_at: None,
        emitted_at: Some(fireweed_core::UtcTimestamp::new(1, 0).unwrap()),
...
fn fjord_dependency_is_git_pinned_no_path_deps() {
    let cargo_toml =
        std::fs::read_to_string(PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("Cargo.toml"))
            .expect("read fireweed-server Cargo.toml");
    ...
    assert!(
        ...
        "fireweed-server must depend on the fjord broker package"
    );
    assert!(
...
fn no_c_kafka_client_dependency() {
    ...
    assert!(
        ...
        "fireweed-server must not depend on the C Kafka client"
    );
    assert!(
