...
use fireweed_objectlog::{AsyncObjectLog, ObjectLogProjectionCommitter};
use fireweed_turso::{TursoConfig, TursoRelational};

#[tokio::test(flavor = "current_thread")]
async fn object_log_commit_crosses_the_real_turso_response_barrier() {
    let root =
        std::env::temp_dir().join(format!("fireweed-objectlog-turso-{}", std::process::id()));
...
async fn reopen_automatically_repairs_append_only_tail_before_later_commit() {
    let base = std::env::temp_dir().join(format!(
        "fireweed-objectlog-turso-reopen-{}",
        std::process::id()
    ));
...

    {
        ...
        let appended = committer
            .commit_replayable(
                ...
                .with_fault(fireweed_engine::RawCommitFault::AfterAppendBeforeApply),
            )
            .await
            .unwrap();
