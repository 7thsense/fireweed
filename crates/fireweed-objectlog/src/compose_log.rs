...
fn maintenance_summary(
    ...
    let stopped_by = match report.stopped_by {
        Some(crate::maintenance::MaintenanceExecutionReason::BudgetExhausted) => {
            Some(fireweed_engine::MaintenanceStopReason::BudgetExhausted)
        }
        Some(crate::maintenance::MaintenanceExecutionReason::EpochChanged) => {
            Some(fireweed_engine::MaintenanceStopReason::EpochFenced)
        }
        Some(crate::maintenance::MaintenanceExecutionReason::RetryableFailure) => {
            Some(fireweed_engine::MaintenanceStopReason::RetryableFailure)
        }
        Some(crate::maintenance::MaintenanceExecutionReason::PermanentFailure) => {
            Some(fireweed_engine::MaintenanceStopReason::PermanentFailure)
        }
        Some(
...
/// holds for the retention-floor trim. Mirrors `fireweed_engine`'s internal `ts_to_ms`.
fn ts_to_ms(ts: fireweed_core::UtcTimestamp) -> i64 {
    ts.seconds
        .saturating_mul(1000)
...
impl fireweed_engine::ControlPlane for ObjectLog {
    fn create_queue(
        &self,
        definition: fireweed_core::QueueDefinition,
    ) -> EngineResult<fireweed_engine::CreateQueueOutcome> {
        self.log.create_definition(&definition)
    }

    fn queue_definition(&self, key: &QueueKey) -> EngineResult<fireweed_core::QueueDefinition> {
    ...
    ) -> EngineResult<Vec<fireweed_core::QueueId>> {
        let mut queues = self
            .log
            .recover_definitions()?
            .into_iter()
...
impl ObjectLog {
    ...
    pub fn branch(
        &self,
        source: &QueueKey,
        branch_def: &fireweed_core::QueueDefinition,
    ...
    pub(crate) fn shared_gc_enqueue(
        ...
        if !self.group_commit {
            return Err(fireweed_engine::EngineError::Unavailable);
        }
        Ok(self
            .log
...
impl ObjectLogMaintenance {
    fn fenced_outcome(expected_epoch: u64) -> DetachedRetentionOutcome {
        DetachedRetentionOutcome {
            expected_epoch,
            summary: fireweed_engine::MaintenanceSummary {
                fenced: true,
                stopped_by: Some(fireweed_engine::MaintenanceStopReason::EpochFenced),
                ..Default::default()
            },
...
impl DetachedLogMaintenance for ObjectLogMaintenance {
    fn execute_retention(
        ...
        let mut summary = fireweed_engine::MaintenanceSummary::default();
    ...
    fn execute_orphan_gc(
        ...
        expected_epoch: u64,
        now_ms: i64,
    ) -> EngineResult<fireweed_engine::MaintenanceSummary> {
...
impl LogStore for ObjectLog {
    ...
    fn recover_definitions(&self) -> EngineResult<Vec<fireweed_core::QueueDefinition>> {
