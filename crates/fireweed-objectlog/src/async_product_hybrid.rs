...
use fireweed_engine::{
    ...
    RenewLeasePort, SeparateReplayCommit, SeparateReplayCommitter, SideRecordPage, TickReport,
...
impl fireweed_engine::ItemMutationPort for AsyncObjectLogHybridBackend {
    ...
    {
        let shard = shard.clone();
        // fireweed-c62067d2: Snorri membership/live-mutation uses ItemMutationPort on
        // open_objectlog_sqlite (hybrid). Plan + append under the queue gate.
        async move {
            ...
            self.engine
                .submit_operation(queue, move || {
                    Box::pin(async move {
                        ...
                        if request.dry_run {
                            let plan = projection.with_store_mut(|p| {
...
mod tests {
    ...
    use fireweed_engine::{
        AddressedMutation, AsyncLogStore, Backend, ClaimCompatibility, ClaimPort, ClaimRef,
        ClaimRequest, CommitEntryOutcome, CommitTransition, CommitTransitionEntry,
        CommitTransitionPort, ControlPlaneStore, DurabilityClass, FinalizeKind, FinalizeOutcome,
        FinalizePort, InstanceFence, ItemMutationOperation, ItemMutationPort, ItemMutationRequest,
    ...
    async fn concurrent_item_mutation_request_id_commits_once() {
        let backend = std::sync::Arc::new(
            AsyncObjectLogHybridBackend::open_memory_log(
                ":memory:",
                flush_zero(),
                0,
                HybridProductConfig::default(),
        ...
        let before = backend
            ...
            .unwrap()
            .unwrap();
        let request_id = RequestId::new("concurrent-hybrid-mutation").unwrap();
