...
pub(crate) fn maybe_trace(inner: Arc<dyn BlobStore>) -> Arc<dyn BlobStore> {
    if std::env::var_os("FIREWEED_LOG_TRACE").is_some() {
        Arc::new(TracedBlobStore(inner))
    } else {
        inner
    }
}
struct TracedBlobStore(Arc<dyn BlobStore>);

fn report(key: &str, bytes: usize, started: Instant, ok: bool) {
    ...
    eprintln!(
        "log_blob class={class} bytes={bytes} put_us={} ok={ok}",
...
impl BlobStore for TracedBlobStore {
    async fn put(&self, key: &str, value: Bytes) -> Result<(), ObjectLogError> {
        ...
        let result = self.0.put(key, value).await;
        report(key, bytes, started, result.is_ok());
        result
    }
    async fn put_chunks(&self, key: &str, chunks: Vec<Bytes>) -> Result<(), ObjectLogError> {
        let bytes = chunks.iter().map(Bytes::len).sum();
        let started = Instant::now();
        let result = self.0.put_chunks(key, chunks).await;
        report(key, bytes, started, result.is_ok());
        result
    }
...
mod tests {
    ...
    async fn tracing_preserves_chunk_range_error_and_accounting_contracts() {
        let store = TracedBlobStore(Arc::new(object_log::MemoryBlobStore::new()));
        store
            .put_chunks(
                "fwlog/test",
