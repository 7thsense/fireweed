//! `fireweed-cost-model` — turn ADR-001's directional S3 cost claim into release EVIDENCE.
//!
...
fn granularity_scenarios() -> Vec<GranularityAssumptions> {
    let scenario = |label: &str,
        ...
        billing_window_hours: fireweed_release::cost::HOURS_PER_MONTH,
...
fn render_granularity_artifact(p: &PriceInputs) -> String {
    ...
    let _ = writeln!(
        ...
        "This document is GENERATED from explicit workload assumptions. It is modelled sensitivity, not \
         measured release evidence. Regenerate it with:\n\n```\ncargo run -p fireweed-release --bin \
         fireweed-cost-model -- --granularity-only --out \
...
fn render_artifact(
    ...
    let _ = writeln!(
        s,
        "This document is GENERATED by `fireweed-cost-model` (a pure calculator in `fireweed_release::cost`). \
...
mod tests {
    ...
    fn atomic_write_failure_preserves_destination_and_cleans_temporary_file() {
        let dir = std::env::temp_dir().join(format!(
            "fireweed-cost-model-atomic-failure-{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&dir);
