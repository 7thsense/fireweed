//! `fireweed-verify-ledger` — strict-validate a verification ledger and assert required evidence is present.
//!
//! Usage:
//!   fireweed-verify-ledger (--ledger <path> | --ledger-dir <dir> | --manifest <path>) [--strict]
...
//! This is the CI gate's evidence check; it rebuilds the binary removed with fireweed-service.
...
fn fail(msg: &str) -> ExitCode {
    eprintln!("fireweed-verify-ledger: {msg}");
    eprintln!(
        "usage: fireweed-verify-ledger (--ledger <path> | --ledger-dir <dir> | --manifest <path>) [--strict] [--require-evidence E0,E1,E2,E3] [--require-smoke-evidence E2,E3]"
    );
    ExitCode::FAILURE
}
