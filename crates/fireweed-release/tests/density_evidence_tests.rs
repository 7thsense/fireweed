...
fn density_loadgen_contains_fail_closed_shape_lifecycle_and_active_load_guards() {
    ...
    assert!(control_plane_execute.contains("fireweed_resp::try_spawn_governed"));
    assert!(control_plane_execute.contains("resource: \"runtime tasks\""));
    assert!(!control_plane_execute.contains("fireweed_resp::spawn_governed"));
...
fn density_resp_xadd_pipeline_uses_bounded_downstream_batches() {
    assert!(RESP_SERVER.contains(
        "PIPELINE_XADD_COMMAND_LIMIT: usize = fireweed_engine::MAX_ORDERED_INDEPENDENT_PUSH_ITEMS"
    ));
