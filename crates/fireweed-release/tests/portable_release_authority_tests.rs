...
use fireweed_release::{LedgerRow, Measurements};
...
fn portable_e0_e1_accept_semantics_not_machine_speed() {
    for id in ["E0", "E1"] {
        fireweed_release::single_deployment::validate_row(&portable(id), id, REV).unwrap();
    }
}
...
fn portable_e0_e1_require_exact_governed_evidence_identity() {
    for id in ["E0", "E1"] {
        let row = portable(id);
        fireweed_release::single_deployment::validate_row(&row, id, REV).unwrap();
        ...
        for evidence_ids in [vec![], vec![id.into(), "E2".into()]] {
            ...
            assert!(
                fireweed_release::single_deployment::validate_row(&malformed, id, REV).is_err(),
...
fn e0_rejects_legacy_nonwrapper_and_noncausal_rows() {
    ...
    for key in [
        ...
        assert!(
            fireweed_release::single_deployment::validate_row(&legacy, "E0", REV).is_err(),
    ...
    assert!(fireweed_release::single_deployment::validate_row(&same_member, "E0", REV).is_err());
}
...
fn e0_e1_reject_quiet_host_and_absolute_speed_bars() {
    for text in ["quiet host required", "throughput >= 2778", "p99 < 1000ms"] {
        ...
        assert!(fireweed_release::single_deployment::validate_row(&row, "E0", REV).is_err());
    }
}
...
fn e0_e1_reject_unmeasured_progress_and_resource_claims() {
    ...
    for key in [
        ...
        assert!(
            fireweed_release::single_deployment::validate_row(&row, "E0", REV).is_err(),
...
fn e0_e1_reject_resources_over_bounds_and_non_maximum_batch_probe() {
    ...
    assert!(fireweed_release::single_deployment::validate_row(&e0, "E0", REV).is_err());

    let mut e1 = portable("E1");
    e1.measurements
        .values
        .insert("configured_max_batch_size".into(), serde_json::json!(2_000));
    assert!(fireweed_release::single_deployment::validate_row(&e1, "E1", REV).is_err());
}
...
fn e0_e1_fail_closed_on_progress_topology_workload_and_reconciliation_drift() {
    ...
    for (key, value) in [
        ...
        row.measurements.values.insert(key.into(), value);
        assert!(
            fireweed_release::single_deployment::validate_row(&row, "E1", REV).is_err(),
            "drift in {key} must fail closed"
        );
    }
}
