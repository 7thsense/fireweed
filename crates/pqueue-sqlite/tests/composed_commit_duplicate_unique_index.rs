...
use pqueue_core::{IndexDeclaration, IndexType, PriorityValue, QueueDefinition, QueueIndex};
use pqueue_engine::{
...
async fn composed_sqlite_log_rejects_in_commit_duplicate_unique_key_and_reopens_clean() {
    ...
    {
        let backend =
            pqueue_sqlite::composed_sqlite_log_sqlite_projection(&log_path, &proj_path).unwrap();
...
async fn composed_sqlite_relational_rejects_in_commit_duplicate_unique_key_and_reopens_clean() {
    ...
    {
        let backend = pqueue_sqlite::composed_sqlite_relational(&path).unwrap();
        run_reject(&backend).await;
    } // drop the handle
    ...
    let reopened = pqueue_sqlite::composed_sqlite_relational(&path).unwrap();
    assert_eq!(
        reopened.metrics(&shard()).await.unwrap().pending,
        1,
        "reopen recovers exactly one lifecycle item"
