...
//!
//! The object-log substrate lives in `pqueue-objectlog` (which `pqueue-sqlite` deliberately does not depend
...
//! `pqueue-conformance/tests/objectlog_hybrid.rs` and `pqueue-objectlog/tests/hybrid_request_id.rs`.

use pqueue_conformance::{item, qdef, shard, ts};
use pqueue_core::{ItemId, RequestId};
use pqueue_engine::{
